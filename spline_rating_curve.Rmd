...
```{r gam_estimate_of_rating, fig.height = 7, fig.cap =  }
...
# Xp_df   <- data.frame(Xp)
 ...
 as.data.frame(Xp) %>%
  ...
  mutate( basis_row = rep(1:776, 10)) %>% 
   ...
   labs( title = 'Figure 7. Basis (matrix columns of Xp) for GAM Smoothing Spline' )
...
data.frame(post_dnorm) %>% 
  ...
  stat_function(fun = dnorm, 
                args = list( mean = mean(post_dnorm$sigma), 
                             sd   = sd(  post_dnorm$sigma)), color = 'red' ) +
  scale_x_continuous( limits = c(0.055, 0.070)) + 
  labs( title = paste0('Figure 6. Distribution of Bayes dnorm parameter sigma with reference dnorm(mean = ',
