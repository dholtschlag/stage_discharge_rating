...
# Use quadratic approximation posterior distribution (quap) 
baye_gam_uncert_x <- map2stan(
  alist(
    lflow_est  ~ dnorm( lflow_rat , sigma ) ,
...
data=list( log_flow  = meas_data$log_flow, 
           ...
           lflow_sig = meas_data$meas_flow_stderr) ,
...
# Parameter estimates from Bayesian estimation
precis( baye_gam_uncert_x, depth = 1, prob = 0.95, digits = 5)

# Extract parameter estimates
post_dnorm   <- extract.samples( baye_gam_uncert_x, n = 1e4 )
# head(post_dnorm)
