...
set.seed(3472792)
meas_data <- meas_data %>% 
  mutate( log_stage_uniq   = log_stage_round + runif(nrow(meas_data), -0.0000049, 0.0000049)) %>% 
  arrange(log_stage_uniq)
...
meas_summary <- meas_summary %>% 
  mutate(stage_rnd = 10^log_stage_round,
  flow_mean = 10^mean_log_flow) %>%  
  dplyr::select(stage_rnd, everything())
...
fit.gamma.logstage <- fitdist(log10(meas_data$stage)-min(log10(meas_data$stage))+.0001,
                              distr = 'gamma', method = 'mle')

summary(fit.gamma.logstage)

meas_data %>% 
  ggplot( aes(log_stage_uniq - min(log_stage_uniq))),  color = 'blue' ) +
...
rating_data %>% 
  ...
  geom_point(data = meas_data, aes(x = stage, y = flow, 
                                  color = Quality)) +
  theme_bw() +
...
```{r gam_estimate_of_rating, fig.height = 7, fig.cap = 'Figure 7. Basis for GAM Smoothing Spline'  }

# Compute GAM 
Gmodel  <- gam(meas_data$log_flow ~ s(meas_data$log_stage_uniq))
...
# Xp_df   <- data.frame(Xp)
 ...
 as.data.frame(Xp) %>%
  ...
  mutate( basis_row = rep(1:776, 10)) %>% 
   ggplot( aes( x = basis_row, y = smo_stage, color = basis)) +
   geom_line() +
   theme_bw()  +
   theme( legend.position = 'bottom') +
   xlab('Basis Row (Sorted by Increasing Stage)') +
...
### Bayesian Estimate of Spline Parameters


```{r Bayes_parameter_estimates, fig.height = 6}
...
ggcorrplot( cov2cor( vcov( baye_gam)), type = 'lower', 
...
# plot( coef(Gmodel), coef(baye_gam)[1:10], pch = 16, col = 'red')
