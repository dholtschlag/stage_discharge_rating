...
meas_data <- meas_data %>% 
  ...
  dplyr::select( 'measurement_nu', 'measurement_dt', 'measurement_tm', 'tz_cd_reported',
                 'stage', 'flow_meas', 'qualifier',  'control_type',
                 'se_pct', 'se_flow' )
...
df_meas_data <- meas_data %>% 
  group_by( measurement_dt ) %>% 
  summarize( flow_meas    = mean( flow_meas) ,
             stage        = mean( stage    ) ,
             n            = n(),
...
df_stage_flow_allDays <-df_stage_flow_allDays %>% 
  left_join( df_meas_data[, c('measurement_dt', 'flow_meas', 'qualifier', 'control_type', 'se_flow')], 
...
ndx_ina <- which( is.na(df_stage_flow_allDays$stage_mean_day ))

# Find dates for missing data
dates_missing <- df_stage_flow_allDays$day[ndx_ina]

# Retrieve daily flows for span of missing data interval
flow_dv_seq   <- readNWISdv( '04122500', parameterCd = '00060', startDate = min(dates_missing),
                               endDate = max(dates_missing))

# Find indices during the flow seqence that represent missing data
ndx_miss      <- which( flow_dv_seq$Date %in% dates_missing )

# Estimate daily mean stage from published daily mean flow
stage_missing <- approx( x = rating19$rated_flow_19, y = rating19$stage, 
                         xout = flow_dv_seq$X_00060_00003[ndx_miss])


df_stage_flow_allDays$stage_mean_day[ ndx_ina ] <- stage_missing$y
...
Yt <- matrix(df_stage_flow_allDays$flow_meas, nrow = 1)
...
df_stage_flow_allDays$ytT    <- as.vector(kemfit_19_Qdiag_equal_fx10$ytT)
df_stage_flow_allDays$ytT.se <- as.vector(kemfit_19_Qdiag_equal_fx10$ytT.se)

# Compare rating with kfgam estimates

stage_knots_19 <- predict( gam_rate_19.0, newdata = data.frame( stage = df_stage_flow_allDays$stage_mean_day),
                                           type = 'lpmatrix')

stage_knots_19 <- predict( gam_rate_19.0, type = 'lpmatrix')



stage_knots_19 <- cbind(rating19.0$stage, stage_knots_19) 
colnames(stage_knots_19)[1] <- 'stage'

tmp <- meas_data %>% 
  left_join( df_stage_flow_allDays, by = c('measurement_dt' = 'day') )

ndx_kfss <- which( df_stage_flow_allDays$day %in% meas_data$measurement_dt )
...
rating19.0[, c('stage', 'rated_flow_19.0', 'flow_kfgam19.0', 'flow_kfgam20.0', 'flow_kfgam20.1',
                    'flow_kfgam21.0', 'flow_kfgam22.0')] %>% 
  rename( flow_rating19.0 = rated_flow_19.0) %>% 
...
df_flow_xtT_stats  <- data.frame( stage = rating19.0$stage, flow_xtT_min_stage = flow_xtT_min_stage,
...
p <- rating_long %>% 
  ...
  ylab( 'Stage, in feet above gage datum' ) + 
  labs(title = 'Stage-Flow Ratings at 04122500 Pere Marquette River at Scottville, MI') +
  geom_line( data = rating19.0, aes( x = rated_flow_19.0, y = stage), color = 'black', linetype = 'dots' )
...
tmp$Date  <- rep( df_stage_flow_allDays$day[ndx_kfss] , each = 731)
