...
Stage-flow (stage-discharge) relations can be described by a table that describe the monotonically increasing relation between stage and flow at a streamgage. This relation has generally been developed manually on the basis of historical discrete, contemporaneous measurements of stage and flow that span the typical range of conditions at the streamgage.  The stage-flow relation can change gradually over time due to systematic changes in the channel or overbank morphology or suddently due to debris or ice formation. 
in persistent or erratic 


## Methods for Developing a Kalman Filter


$$
\begin{align}
    \text{State Equation:  }  x_{t} = x_{t-1} + w_t && \text{where } w_t \text{~} MVN(0, Q) \text{,}\\
    \text{Measurement Equation:  } y_{t} = Z_t \text{ } x_{t} + v_t && \text{where } v_t \text{~} MVN(0, R_t) \text{,}\\
    \text{}
\end{align}
$$
...
```{r show_rating_history, echo = FALSE}
...
```{r plot_ratings, fig.width = 9, fig.height = 8,}
...
```{r plot_gam_error, echo = FALSE }
rating_init %>% 
  # dplyr::arrange( stage ) %>% 
  ggplot(    aes( y = gam_error, x = stage  )) +
  ...
  theme( legend.position = 'bottom') +
  labs( title = paste0('Figure ', fig_no, '. Errors in Flow for GAM of rating curve 19 at ', site_info$site_no, 
...
gam18_model <- gam( flow_discrete_mea ~ s(stage_discrete_mea), 
                     weights = wt,
                     data = discrete_meas_wt[ discrete_meas_wt$rating == 18, ],
                     family = Gamma(link = log))

gam19_model <- gam( flow_discrete_mea ~ s(stage_discrete_mea),
                     weights = wt, 
                     data = discrete_meas_wt[ discrete_meas_wt$rating == 19, ],
                     family = Gamma(link = log))

gam20_model <- gam( flow_discrete_mea ~ s(stage_discrete_mea),
                     weights = wt, 
                     data = discrete_meas_wt[ discrete_meas_wt$rating == 20, ],
                     family = Gamma(link = log))

gam21_model <- gam( flow_discrete_mea ~ s(stage_discrete_mea),
                     weights = wt, 
...
gam_long %>% 
  ...
  theme_bw() +
  scale_color_manual( values = c('cyan', 'cyan', 'navy', 'red') ) +
...
```{r unit_daily_stage, echo = FALSE, warning = FALSE, message = FALSE}

stage_unit <- read.csv(file = 'Data/04122500/stage/unit_stage.csv',
                       comment.char = '#', 
                       stringsAsFactors = FALSE, header = TRUE)

colnames(stage_unit)[2]   <- 'datetime'
# Convert from character field to datetime referenced to EST
stage_unit$datetime <- as_datetime(stage_unit$datetime, tz = 'America/New_York')
...
stage_missing <- approx( x = rating19$rated_flow, y = rating19$stage, 
...
Zt <- t(predict( gam_model, newdata = data.frame( stage = df_stage_flow_allDays$stage_mean_day),
                                           type = 'lpmatrix'))
# Remove row names
rownames(Zt) <- NULL
...
X0 <- as.matrix( as.numeric(coefficients(gam_model)), 10, 1)
