...
colnames( rating19 ) <- c('rated_flow_19.0', 'stage')

rating20 <- read.csv('Data/04122500/04122500_rating_20.0.txt', header = FALSE, sep = '\t') 
colnames( rating20 ) <- c('rated_flow_20.0', 'stage')
...
inits = list(Q = Qtry)
par_list <-coef(kemfit_r, what ='par')

Qtry <- array( c(1200, 0.1, 0.002, 0.002), dim = c(4,1), dimnames = list(c('vvii', 'vvkk', 'cvik', 'cvkk'), c(NULL)))
...
for ( i in 1:10) {
  if (!strcmp( unlist(Q[i,j]), unlist(Q[j,i]))){
    print(paste(i,j))
  }
}
...
kemfit_por <- MARSS(Yt, model = model.gen, control = list(maxit = 2000 ))
...
# paramCIs_hessian    <- MARSSparamCIs(kemfit_por, method = 'hessian')

# Errors displayed in console, but could not interrupt without shutting down. 
# paramCIs_parametric <- MARSSparamCIs(kemfit_por, method = 'parametric')
...
tmp <- MARSSboot( kemfit_por, nboot = 1000, output = 'parameters', sim = 'innovations', param.gen = 'MLE' )
