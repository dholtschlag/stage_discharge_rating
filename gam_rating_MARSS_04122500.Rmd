...
Yt <- matrix(stage_flow_day_19$flow, nrow = 1)

# Predict the intercept and slope components for the spline smooth 
Zt <- t(predict( gam_rate_19, newdata = data.frame( stage = stage_flow_day_19$stage_mean_day),
                                           type = 'lpmatrix'))
...
Qc <- matrix(NA, 10, 10)
# Convert Q matrix to list 
for (i in 1:10){
  for (j in 1:10) {
    Qc[i,j] <- paste(format(Qn[i,j], digits = 8, trim = TRUE),' q',  sep = '' )
    # print(Qc[i,j])
  }
}
noquote(Qc[4,5])
...
kemfit <- MARSS(Yt, model = model.gen)
