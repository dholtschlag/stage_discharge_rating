...
```{r plot_basis_functions, fig.width = 9, fig.height = 7}


# Basis functions

basis18 <- predict( gam18_model, newdata = data.frame( stage_discrete_mea = rating18$stage ),
                                           type = 'lpmatrix')
df_basis18 <- data.frame(rep('18', nrow(rating18)), rating18$stage, basis18) 
colnames(df_basis18) <- c('Rating', 'Stage', 'Intercept', 'basis1', 'basis2', 'basis3', 'basis4')
                         
basis19 <- predict( gam19_model, newdata = data.frame( stage_discrete_mea = rating19$stage ),
                                           type = 'lpmatrix')
df_basis19 <- data.frame(rep('19', nrow(rating19)), rating19$stage, basis19)
colnames(df_basis19) <- c('Rating', 'Stage', 'Intercept', 'basis1', 'basis2', 'basis3', 'basis4')

basis20 <- predict( gam20_model, newdata = data.frame( stage_discrete_mea = rating20$stage ),
                                           type = 'lpmatrix')
df_basis20 <- data.frame(rep('20', nrow(rating20)), rating20$stage, basis20) 
colnames(df_basis20) <- c('Rating', 'Stage', 'Intercept', 'basis1', 'basis2', 'basis3', 'basis4')

basis21 <- predict( gam21_model, newdata = data.frame( stage_discrete_mea = rating21$stage ),
                                           type = 'lpmatrix')
df_basis21 <- data.frame(rep('21', nrow(rating21)), rating21$stage, basis21) 
colnames(df_basis21) <- c('Rating', 'Stage', 'Intercept', 'basis1', 'basis2', 'basis3', 'basis4')

df_basis <- rbind(df_basis18, rbind(df_basis19, rbind(df_basis20, df_basis21)))
...
kem_fit_c <- MARSS(Yt, model = model.gen, control = list(maxit = 100),inits = coef(kem_fit_b))
...
kem_fit_a <- MARSS(Yt, model = model.gen, control = list(maxit = 200),inits = coef(kem_fit_Qdiag_Rseries_SE))
...
kem_fit_df <- data.frame( Model = c('Q and R estimated', 'Q estimated, R specified'), 
                   'Q'              = format( c(kem_fit_Qdiag_Rscalar$par$Q,
                                                kem_fit_Qdiag_Rseries_SE$par$Q), digits = 4),
                   ...
                   'Log_likelihood' = format(c(kem_fit_Qdiag_Rscalar$logLik, 
                                               kem_fit_Qdiag_Rseries_SE$logLik), digits = 4),
                   'AICc'           = format(c(kem_fit_Qdiag_Rscalar$AICc,   
                                               kem_fit_Qdiag_Rseries_SE$AICc), digits = 4))
                                               
                   
kem_fit_df %>% 
  kable( caption = 'Table 5. Summary statistics from Multivariate Autoregressive State-Space estimation.') %>% 
  kable_styling( position = 'center') %>% 
  add_header_above( c(' ', 'Variance' = 1, 'Variance' = 1, 'Standard deviation' = 1, ' ' = 2), align = 'l') %>% 
...
df_stage_flow_allDays %>% 
  ...
  ggplot( aes( x = Date )) +
  geom_line( aes( y = exp(cl025) ), color = 'blue') +
  geom_line( aes( y = exp(cl975) ), color = 'blue') +
  theme_bw()   + 
  scale_y_log10() +
  ...
  geom_point( aes( y = flow_discrete_mea   ), color = 'darkblue', size = 2) 
...
```{r eval_state_space, echo = FALSE, fig.width = 11, fig.height = 16}
...
design_matrix_allDays <- predict( gam20_model, newdata = data.frame( stage = df_stage_flow_allDays$stage_daily_mean),
                                           type = 'lpmatrix')
...
# Design matrix for allDays
design_matrix_allDays <- predict( gam21_model, newdata = data.frame( stage_discrete_mea = 
...
# Preallocate matrix to contain time-varying spline parameters at times of DFM
flow_xtT <- matrix(NA, nrow = nrow(rating21), ncol = length( ndx_DFM ) )


design_matrix <- as.matrix(design_matrix_rating[,2:6])

for (i in 1:length(ndx_DFM )){
  flow_xtT[ , i ] <- exp( design_matrix_rating %*% matrix( kem_kfss$xtT[ , ndx_DFM[ i ]], 5, 1 ) )
}
