...
rating17 <- read.csv('Data/04122500/04122500_rating_17.0.txt', header = FALSE, sep = '\t') 
colnames( rating17 ) <- c('flow', 'stage')
...
rating17 %>% 
  ...
  ylab( 'Stage, in feet') +
  labs(title = paste('Figure 1. Stage-Flow relation 17.0 at', site_no, station_nm)) +
  theme_bw() 
...
stage_day <- stage_unit %>% 
  ...
  summarize( day_mean  = mean( stage, na.rm = TRUE ),
             n         = n()) %>% 
  ungroup() %>% 
  arrange( day ) %>% 
