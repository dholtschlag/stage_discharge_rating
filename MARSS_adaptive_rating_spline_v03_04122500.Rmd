...
A stage-flow relation is adapted by changing CRS parameters on the basis of discrete measurements of stage and flow.  Intervening measurements of stage, however, can be used to generate a set of basis functions (one for each knot) from an underlying CRS and used to compute the magnitudes and uncertainties of daily or unit streamflow. 
...
df_stage_flow_allDays[, ] %>% 
  ...
  ggtitle( paste0('Figure ', fig_no, '. Time Series of daily mean stage computed from unit values at \n04122500 Pere Marquette River at Scottville, Mich., during the period of analysis'),
           subtitle = paste0('Vertical blue lines indicate divisons between selected rating periods.'))
...
# Methods for Adapting Stage-Flow Rating and Estimating the Magnitude and Uncertainty of Streamflow Records

Using a generalized additive modelling (GAM) framework[@Wood2006], a cubic regression spline (CRS) is used to approximate either an existing rating from an extended rating table or develop a rating from a set of discrete stage-flow measurements representing typical flow conditions at a streamgage.  Parameters of the CRS form the state vector of the state equation, which together with the measurement equation form the state-space model. In this analysis, parameter estimates in the state-space model are computed at daily time steps. In applications, unit time steps could be used. 

The uncertainty of the state vector increases with time between measurements based on an estimated process covariance matrix $Q$ in the state equation. Within the measurement equation, the state vector is premultiplied by a set of basis functions with compatible length (computed as a function of stage), to estimate measured flow. On days when a discrete measurement occurs, the parameters in the state vector are updated to adapt the rating curve to reduce the differences between the estimated and measured flows, and to reduce the uncertainty described by the state covariance matrix $V$.  

The remaining discrepancies between discrete measurements of flow and estimated flow, both in natural logarithmic units, are described by the measurement variance $R$ in the measurement equation. The value of $R$ can be specified or estimated iteratively. Once, $Q$ and $R$ are obtained, a set of CRS parameters can be computed that sequentially adapt the rating curve based on each discrete flow measurement used in the analysis. In addition, a Kalman filter [@Simon2006] can be applied to estimate the magnitude and uncertainty of daily or unit flows based on daily or unit stages. Details of this process are described in the following paragraphs. 
...
marss_state_tT %>% 
  ...
  ggtitle( label = paste0('Figure ', fig_no, '. Step plot of discrete changes in the state vector of basis parameters on days of discrete measurements.') ,
