...
## Appendix 3. Maximum likelihood and bootstrap estimates of the magnitude and uncertainty of diagaonal elements of the process covariance matrix Q and measurement variance R.

The purpose of this analysis was to investigate possible bias in the maximum likelihood (ML) estimates of the diagonal and unequal elements of the process covariance matix Q. The positive bias aries because variance is contrained to be positive. This bias is more severe when elements are very small, as indicated by confidence intervals that span zero.  According to the [@MARSS2020b], this happens when there is not sufficient data to estimate both process covarainces and measurement variances specified. The *MARSSboot* function was used to provide unbiased bootstrap parameter estimates of magnitude and uncertainty for comparison with maximum likelihood estimates.  
...
The estimated Q and R parameter magnitudes (plotted vertically) and uncertainties (plotted horizontally) are shown on fig. `r fig_no + 1`. In addition to the ML estimates, there were two bootstrap samples, each of size 1000. The general consistency between intervals positions and expected values for the two samples support the reproduciblity of the results.  
