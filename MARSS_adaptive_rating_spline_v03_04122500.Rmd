...
Cubic regression splines (CRSs) estimated in a generalized additive modelling (GAM) framework provided a basis for developing a set of stage-flow ratings from data in extended rating tables, and from data in sets of discrete stage-flow measurement pairs. A set of five parameter (five knot) CRSs was developed with corresponding static linear basis functions, determined from stage values. The CRSs provided equations for predicting flow at these initial conditions. Parameters from the CRS developed by use of the extended rating table 18 was selected to represent initial rating conditions. 
...
The CRS parameters were made time varying to adapt the initial rating to persistent changes in the stage-flow relation using discrete measurements within a state-space modelling framework.
