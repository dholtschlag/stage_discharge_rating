...
The *model.matrix* function can be used to create a design matrix $Z$ from a GAM object as: $Z$ = model.matrix([gamObject]). The gamObject can be any gam object that received input from the gam() function specifying the model structure and data contents. In this usage, the design matrix $Z has dimension T \times 5$ is composed of a set of 5 basis values for each stage value provided to the CRS model. In the case of a rating table, T corresponds to the number of rows in the extended rating table. In the case where a rating is developed by fitting discrete stage and flow measurements, *T* is the number of discrete measurements associated with the rating period. Alternately, the **predict** function can be used to create a design matrix $Z$ from a GAM object and a new R data.frame that have a variable named 'stage' as $Z = predict( [gamObject], newdata = data.frame( stage = ... ) )
...
Similarly, $N(0, R_{t})$ indicates a *Normally* distributed error with mean $0$ and scalar or time series measurement variance $R_t$.  

#### State Vector $\vec{x}_t$

Vector $\vec{x}_t$ is the time series of state vectors, where each vector has a length of 5, corresponding to the number of knots specified for the CRS. The state equation models the transition of the state vector from time $t-1$ to $t$ as a random walk at a daily time step. The state-vector is invariant between discrete flow measurements, when the measurement update is skipped because of missing value (NA) in the daily flow series $y_t$. The covariance of the state vector before the measurement update $V_t^-$, however, is incremented by $Q$ at each time step in the state equation, while  $V_t^+$ is state variance updated (reduced) only when discrete flow measurements are indicated in the $y_t$ vector. 
