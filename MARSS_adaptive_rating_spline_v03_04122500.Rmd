...
CRS are piecewise polynomials with constraints to ensure continuity to the second derivative at interior knots and zero second derivatives at the boundary knots [@Wood2006].  Individual polynomials are cubic (third order). Special constraints on the boundary knots ensures linear extrapolation beyond the domain of the stage data. CRS parameters were estimated within a GAM to flexibly represent the monotonically increasing relation between stage and flow. The GAM package 'mgcv' [@mgcv2019] in R provided parameter estimation, statistics on model fit, and utility functions like *gam()*, *matrix.model()* and *predict()* for developing the CRSs within the R [@cite_R] programming environment. 

A simple cubic polynomial regression will be discussed to provide a more familiar frame of reference before discussing CRS. A cubic polynomial regression can be specified by a set of parameters $\beta_j$ and polynomial terms (in place of basis functions) $x^j$ for $j = 0, 1, 2, 3$, such that $\hat{y}_i = \beta_0 + x_i \beta_1 + x_i^2 \beta_2 + x_i^3 \beta_3.$ Although a cubic polynomial is often a useful approximation for point estimation, CRS are preferred, however, when estimating the response over a range of values [@Wood2017]. 
...
Estimating $Q$ and $R$ at a daily time step reduces the precision of the timing of discrete measurements. In particular, a daily time step does not provide the temporal resolution to differentiate between a measurement taken at 9:00 AM and 4:00 PM, which allows for travel time to and from the streamgage.  This time interval likely spans the daylight hours when most discrete measurements are obtained. This fractional part of the day, however, is thought to have little impact on parameter estimates when past discrete measurements had an average time between measurements of 56.9 days apart.  

The process covariance matrix $Q$ increments the state covariance matrix $V_k$ to reflect the increased uncertainty of the forecasted state vector from time $k-1$ to $k$. According to [@MARSS2020a], the maximum likelihood estimates of $Q$ and $R$ are fundamentally biased, and this bias is more severe when either $Q$ or $R$ are small. This bias arises because variance is constrained to be positive, and this variance does not go to zero with increasing sample size. A daily time step helps constrain the effect of this bias by providing estimates of $Q$ that are as large as practical while approximating the timing of discrete measurements. This discretization is thought to be adequate for describing the uncertainty of parameters of the rating curve. When estimating the uncertainty of unit flow magnitudes, however, the value of $Q$ value estimated at daily time steps can be divided by the number of unit time steps per day to better approximate the increase in variance of flow at unit time intervals.  
...
$V_{k}^{-} = E[(x_{k} - \hat{x}_k^{-}) \ (x_{k} - \hat{x}_k^{-})^{'}]$, *a priori* covariance of the state estimation error (a priori state covariance),  
$V_{k}^{+} = E[(x_{k} - \hat{x}_k^{+}) \ (x_{k} - \hat{x}_k^{+})^{'}]$, *a posteriori* covariance of the state estimation error,  
...
|:  $\vec{z}_k$                 |: $\vec{z}_k$ is the design array formed by the daily time series of basis functions. The basis functions are derived from the CRS based for the GAM rating 18 computed using the rating table. Daily values for the basis function were predicted using daily mean stage, except on days with discrete measurements. On these days, the discrete measured stage was substituted for the daily mean value.                                                                                         |  
...
# Design Array formed with from Rating 18 basis functions using the stage range in Rating 21
