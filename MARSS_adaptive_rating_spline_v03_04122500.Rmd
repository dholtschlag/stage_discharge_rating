...
Some discrete measurements were removed from the analysis because they were considered to be outliers with respect to persistent changes in the open-water stage-flow relation. These included all measurements of control type 'Ice Covered' and measurement numbered 474, and 657, obtained on Jan. 28, 1986, and Dec. 11, 2013, respectively. Both measurements indicated the control type as 'Shore Ice,' but had backwater departures more consistent with 'Ice Covered' measurements. The two measurements were qualified as 'Fair', and 'Poor,' respectively.  
...
table( discrete_meas$control_type, discrete_meas$qualifier ) %>%
  ...
  footnote('"Check" measurements, which are repeated flow measurements on the same day to confirm the accuracy of the initially measured flow, were made on two days during the period of analysis. An average flow was computed from the two measurements on the same day to represent the discrete flow measurement on that day.')
...
A cubic regression spline (CRS) was used to model rating 18 at 04122500 Pere Marquette River at Scottville, Michigan. This resulting CRS provided initial parameters and basis functions (determined from stage values) that was adapted to conditions consistent with ratings 19, 20, and 21. The adaptations of initial parameters and corresponding rating was based on tracking discrete stage-flow measurement at daily time steps using a state-space model with Kalman Filtering. The Kalman Filter (KF) also provided a basis for characterizing the uncertainty of a daily streamflow record. The following section describes the development of the CRS and state-space modelling approach.  

## Cubic Regression Splines

CRS are piecewise polynomials with constraints to ensure smoothness, as continuity of second derivatives at interior points (knots), and linear extrapolation beyond the minimums and maximums of discrete stage measurements [@Wood2006].  Individual polynomials are cubic (third order). CRS parameters were estimated within a Generalized Additive Modelling (GAM) framework to flexibly represent the monotonically increasing relation between stage and flow. The GAM package 'mgcv' [@mgcv2019] in R provided parameter estimates, statistics on model fit, and utility functions like *gam()*, *matrix.model()* and *predict()* that facilitated the development of CRSs within the R programming environment. 

As an introduction, a simple cubic polynomial regression may provide a familiar model construct. A cubic polynomial regression can be specified by a set of parameters $\beta_j$ and polynomial functions $x^j$ for $j = 0, 1, 2, 3$, such that $\hat{y}_i = \beta_0 + x_i \beta_1 + x_i^2 \beta_2 + x_i^3 \beta_3.$  Although a cubic polynomial regression is often a useful approximation for point estimation, CRS are preferred when estimating the response over a range of values [@Wood2017].  

CRSs used in this analysis were specified as having 5 knots, which was thought to be sufficiently flexible for the stage-flow relations at the selected streamgage. Five knots resulted in two boundary knots at the minimum and maximum of the stage data, and three interior knots at the 25^th^-, 50^th^-, and 75^th^-percentiles. Generally, the *gam()* function defaults to placing the interior knots at evenly-spaced quantiles of the data, although knot locations can be user-specified to coincide with apparent breaks in the curvature of the rating. The basis functions themselves are not affected by the magnitude or distribution of the response variable (flow).  
...
In the measurement equation [2], the transitioned (column) state vector $\vec{x}_{k}$ is premultiplied by a row vector $\vec{z}_k$ that is the five-element basis function, which forms the design array time series $\vec{z}_k$. The product is the state-space model estimate of flow at time $k$. 
...
In *R*, a design vector $\vec{z}_k$ is extracted from a 1 $\times$ 5 $\times$ 10957 design array $Z_k$ described by the number of levels (equal to 1), rows (equal to 5), and columns (equal to the number of days in the period of analysis), respectively. Because the number of levels equals 1, this design array $Z_k$ is effectively a matrix, although the array form is used in the MARSS package.  Each column vector of length five in the design array provides a set of basis functions $\vec{z}_k$, derived from the corresponding daily stage value. Basis functions are computed from the CRS model for rating 18. On days of discrete measurements, the stage value used for to generate the basis functions is the discrete measurement of stage. Otherwie, the daily mean stage is used to compute the basis functions.   
