...
Figure `r fig_no + 1` shows the time series of changes in Kalman filter and smoother estimates of basis parameters with times of discrete measurements, which are indicated by vertical grey lines. Note that the filter estimates are shown as step plot because parameter estimates only change at times of discrete measurements. In contrast, the smoother estimates change linearly between measurements because information before and after the time of estimation are used in the computation. 

All parameter series followed a random walk, which is expected because an identity matrix premultiplies $\vec{x}_{t-1}$ in the state equation. The parameter associated with the intercept term, $\beta_0$, generally increases with time, and the filter and smoother estimates are in close agreement. The remaining parameters, $\beta_1 - \beta_4$, tended to take their minimum values at the end of the period of analysis. In addition, the match between filter and smoother estimates generally decreases with the higher parameter numbers. The beginning and ending points for all parameters are nearly the same. The differences in filter and smoother estimates may reflect parameter sensitivity. At the end of the period of analysis, the maximum correlation of 0.688 occurred between parameters $\beta_0 \ and \ \beta_3$, and the minimum correlation of -0.360 occurred between parameters $\beta_2 \ and \ \beta_4$. The parameter time series are based state-space Model 9.    
...
df_stage_flow_allDays %>% 
  ...
  suppressWarnings({geom_point(aes( y = flow_discrete_mea,   color = qualifier), size = 1.5 )}) +
  suppressWarnings({geom_point(aes( y = flow_discrete_mea ), size = 5, shape = 1, color = 'black'  )}) +
