...
```
The R package MARSS [@MARSS2020b] was used in developing the state-space model. The notation below follows the authors' notation. 

### State-Space Model

The state-space model [@MARSS2020a]
...
marssBoot <- MARSSboot( marss_fit, output = 'parameters', nboot = 5000, sim = 'parametric', param.gen = 'hessian')
...
marssBootdf <- data.frame( Qdiag = as.numeric( marssBoot$boot.params ) )

marssBootdf %>% 
  ggplot( aes( Qdiag )) +
  geom_density() +
  theme_bw() +
  geom_vline( xintercept = marss_fit_Q_ones_multiplier_R_series_qualifierCI$parMean, color = 'blue') +
  geom_vline( xintercept = c(marss_fit_Q_ones_multiplier_R_series_qualifierCI$par.upCI$Q,
                             marss_fit_Q_ones_multiplier_R_series_qualifierCI$par.lowCI$Q),
              linetype = 'dashed', color = 'blue' ) +
  geom_vline(xintercept =  mean(marssBootdf$Qdiag), color = 'red')
