...
USGS streamgage 04122500 Pere Marquette River at Scottville, Mich. was selected for this study. (fig. 1). The Pere Marquette River drains a basin in the west central lower peninsula of Michigan.  The moderately sized basin has a drainage area of 681 mi^2^ at the streamgage. The basin is heavily forested and includes parts of the Huron-Manistee National Forest.  The city of Scottville, population 1,214, and the village of Baldwin, population 1,208,  (https://factfinder.census.gov/faces/nav/jsf/pages/community_facts.xhtml, accessed Feb. 1, 2020), are the primary population centers in the basin.  Streamflow has been computed continuously at the site starting in August 1939 and was active at the time of this paper (October 2020).

This particular streamgage was selected because it has an extended period of streamflow measurements and records to support the analysis. In addition, the stage-flow relation has shown a persistent (and conspicuous) trend that generally enables a lower stage to pass a higher flow than earlier stage-flow relations generally indicate. In addition to the persistent trend, intermittent seasonal ice affects periodically increases the stage needed to pass a given flow than during open-water conditions. The sand channel has shown considerable scour potential. For example, on Sept. 13, 1986, a high flow of 6,440 ft^3^/s was measured at the streamgage, with a maximum local streambed scour of 1.55 ft. Four days later, a second discrete measurement flow measurement of 2,610 ft^3^/s was obtained, had maximum local streambed scour of 4.06 ft [@holtschlag1998]. The utility of the proposed approach for tracking changes in the stage-flow relation, however, is not thought to require such a conspicuous trend.  
...
The probability density describes the frequency characteristics of daily flow magnitudes.  Daily flows at Pere Marquette River during the period of analysis varied from `r flow_hpdi[[1]]` and `r flow_hpdi[[2]]` ft^3^/s, 95-percent of the time.  The minimum and maximum daily flows during this period were `r min(flow_daily_pub$Flow)` and `r max(flow_daily_pub$Flow)`, respectively. The mean flow was 787 ft^3^/s. The probability densities of the flows are asymmetric (right skewed). The natural logarithm of flows, however, is consistent with a three parameter gamma distribution (fig. 2). This distribution is an important consideration in specifying a data transformation to track changes in stage-flow relations.   
...
df %>% 
  ...
  geom_histogram( fill = 'grey', alpha = 0.50, binwidth = 5 ) +
  theme_bw() +
  ...
  annotate('text', x = 49, y = 25.5, label = paste0('Mean = ', format(mean(df$days_between_measurements), digits = 3) , 
                                                    ' days'), hjust = -0.25, vjust = 0, color = 'blue')
...
A linear form of the CRS was used by specifying a set of basis functions such that $b_j(x)$ is the $j^{th}$ function of the set.  A second-order (quadratic) polynomial basis can be represented with basis functions $b_0(x) = 1$, $b_1(x) = x$, $b_2(x) = x^2$ so that $y_i = \beta_0 + x_i \beta_1 + x_i^2 \beta_2 + e_i.$ Thus, a second order polynomial has three basis functions ${1, \ x, \ x^2}$ and three corresponding parameters ${\beta_0, \ \beta_1, \ \beta_2}$.  According to [@Wood2017], a polynomial basis is a useful approximation when interest in on a specific point, however, splines are preferred when interest in the response (flow) is over the entire domain of stage. 

CRSs used in this analysis were specified as having 5 knots, which was thought to be sufficiently flexible for the stage-flow relations at the selected streamgage. This resulted in two boundary knots at the minimum and maximum of the stage data, and three interior knots at the 25^th^-, 50^th^-, and 75^th^-percentiles. The convention of placing the knots at quartiles of the data can be replaced with manual specification more consistent with apparent breaks in the curvature of the rating. The basis functions themselves are not affected by the response variable (flow).  
...
function **gam**
Within the R package *mgcv*, the **gam** function produces a gam output object (gamObject) that contains the parameters, statistics, and input data of a gam model based on the function arguments describing the model structure and providing data for estimation. The object to the left of the tilde (~) is the response variable (rated_flow), the object to the right is the model specification, the data, and  


gamObject <- gam( rated_flow        ~ s(stage,             bs = 'cr', k = knts),  
                       data = data.frame( stage = ..., rated_flow = ... ), family = Gamma(link = log))  
...
|:  ~                 |: Relational operator                                                                                      |  
|: s( ... )           |: Function used in definition of smooth terms within gam model formula. The function does not evaluate    |  
...
The *model.matrix* function can be used to create a design matrix $Z$ from a GAM object as: $Z$ = model.matrix([gamObject]). 
The gamObject can be any gam object that received input from the gam() function specifying the model structure and data contents. In this usage, the design matrix $Z has dimension T \times 5$ is composed of a set of 5 basis values for each stage value provided to the CRS model. In the case of a rating table, T corresponds to the number of rows in the extended rating table. In the case where a rating is developed by fitting discrete stage and flow measurements, *T* is the number of discrete measurements associated with the rating period. 

function **predict**  
The **predict** function can be used to create a design matrix $Z$ from a GAM object and a new R data.frame that have a variable named 'stage': $Z = predict( [gamObject], )
...
Daily time steps were used in this paper because the time between discrete flow measurements averaged 58 days. On average, therefore, there is one measurement update for each 58 state updates in the alternating solution of the state space model. Estimating state-space model parameters at unit intervals would increase the computational burden of solving the state equation by one or two orders of magnitude using hour or 15-minute time steps were used. In addition, the estimation results may be degraded. In particular, [@marss2018] note that when $Q$ (or $R$) gets very small, the bias in the maximum-likelihood estimates of variances becomes more severe. The magnitude of $Q$ is generally small ($<10^{-05}$) even at daily time steps. Alternatively, one might estimate $Q$ at daily time steps and divide $Q$ by the number of time steps in a day for simulation. 
...
Figure `r fig_no+ 1 ` shows the distribution of Kalman forecast $y_t^m - \hat{y}_t^{t-1} $, filtered $y_t^m - \hat{y}_t^{t} $, and smoothed errors $y_t^m - \hat{y}_t^{T} $, where $y_t$ equals the natural logarithm of flow at time $t$, the superscripts refers to the information available at the time of measurement.  Here, $m$ implies the discrete fow measurement, $t-1$ implies one day prior to the discrete measurement, $t$ implies the time of the discrete measurement, and $T$ implies all time in the period of analysis. The forecast error reflect the largest uncertainty (greatest difference between the 75^th^ and 25^th^ percentiles because the forecast is based only on information availble the day before the corresponding discrete measurement. The filtered estimate includes information up to and including information on the day of measurement. The smoothed estimate contains information available throughout the period of analysis, but shows little change in uncertainty from the filtered estimate.  Thus, there is little transfer of information to information at time $t$ from information at time $t+E[\Delta{t}]$, where $E\Delta{t}$ equals 56.6 days (fig. 4).   
...
print( marss_fit_Q_equalvarcov_R_mean_qualifier)
...
print(marss_fit_Q_ones_multiplier_R_mean_qualifier)
...
print(marss_fit_Q_covar_multiplier_R_mean_qualifier)
...
print(marss_fit_Q_unconstrained_R_mean_qualifier)
...
print(marss_fit_Q_covar_multiplier_R_one_multiplier)
...
print(marss_fit_Q_unequal_scalars_R_one_multiplier)
...
print(marss_fit_Q_ones_multiplier_R_one_multiplier)
...
print(marss_fit_Q_covar_multiplier_R_series_qualifier) 
...
print(marss_fit_Q_ones_multiplier_R_series_qualifier)  
