...
A state-space model consists of two equations: a state equation [1], and a measurement equation [2], which can be written in either discrete or continuous forms.  The discrete-time state-space model is presented here because continual water-level measurements are obtained at unit (discrete) intervals, and computations used in state-space modeling are made by use of digital computers. The continuous-time state-space model is the limiting case of the discrete-time state-space model as the length of the time step from $k-1$ to $k$ ($\Delta{k}$) tends to zero [@Simon2006].


$$
\begin{align}
\text{ State Equation:  } \vec{x}_{k}  &= \: B\ \vec{x}_{k-1} +  \vec{w}_{k-1} && \text{where } \vec{w}_{k-1} \sim MVN(0, Q_d) \text{,}\tag{1}\\  
 \text{Measurement Equation:  } y_{k} &=     \vec{z}_{k}{'} \text{ } \vec{x}_{k} + v_t && \text{where } v_t \sim N(0, R_{k}) \text{,} \tag{2}\\
 \text{}
\end{align}
$$
...
[@Simon2006] states that the continuous-time process covariance $Q_c$ is approximately equal to the discrete-time process covariance $Q_d$ divided by the length of the time step $\Delta{k}$, when $\Delta{k}$ is sufficiently small.  This equation provides a basis to approximate the change in $Q_d$ at different $\Delta{k}s$. 

$$
\begin{align}
\ Q_c &\approx \frac{Q_d}{\Delta{k}} \text{,}\tag{3}\\
\text{where } Q_c  &= \text{ is the continuous-time process covariance,} \\
Q_d &= \text{ is the discrete-time process covariance associated with a particular time step } $\Delta{k}$ \text{,} \\
\ \Delta{k} &= \text{the length of the discrete time step.}\\ 
\end{align}
$$

In this analysis, parameters for the daily process covariance matrix $Q_d \equiv Q_day$  were estimated by using daily time steps for the 10,957 days within the 30-year period of analysis using intervening discrete stage-flow measurements. Based on equation [3], elements of the $Q_d$ matrix can be adjusted based on the size of $Delta{k}$ to compute streamflow records at unit of the stage data (equations [5] and [6]).  
...
Estimating $Q$ and $R$ at a daily time step reduces the precision of the timing of discrete measurements,  This strategy avoided the need to estimate  referenced as $Q_{day}$.  
It follows from equation [3] that discrete process defined  $Q_{hour} \approx Q_{day}/24$, or that $Q_{15-min} \approx Q_{hour}/4$ would be appropriate for simulation of unit streamflow data given that $\Delta{k}$ is infinitesimal for $Q_c$.  This implies that elements of $Q_{day}$, derived from parameter estimation in MARSS, can be divided by the number of unit intervals in a day, to simulate flow magnitudes and uncertainties based on unit stage values. 
...
In the measurement equation, the transitioned state vector $\vec{x}_{k}$ is premultiplied by a row vector $\vec{z}_k$ that is the five-element basis function, which forms the design array time series $Z_k$. 
...
The random measurement noise scalar time series $v_{k}$ is assumed to be normally distributed with mean 0 and variance $R$. The measurement variance $R$ reflects the uncertainties in discrete measurements of flow.  The scalar value of R can be thought of as a constant for all discrete measurements, or as a time series with accuracies varying among measurements. There are many factors that can affect the accuracy of a streamflow measurement including the type and condition of equipment used, the characteristics of the flow measurement section, how rapidly the stage is changing during the flow measurement, the presence of ice or debris, other environmental conditions during the measurement, and the experience and subjective evaluation by the hydrographer making the measurement [@rantz1982]. The difficulty in quantifying the uncertainty in the field is related to subjectively and consistently assessing the accuracy of the measurement. 
...
$\hat{x}_{k}^{-} =  E[x_{k}|y_1, y_2, ..., y_{k-1}]$, *a priori* state estimate at time *k*,  
$\hat{x}_{k}^{+} =  E[x_{k}|y_1, y_2, ..., y_{k}]$, *a posteriori* state estimate at time *k*,  
$\hat{x}_{0}^{+} = E[x_{0}]$, initial *a posteriori* state estimate at time = 0,  
$V_{k}^{-} = E[(x_{k} - \hat{x}_t^{-}) \ (x_{k} - \hat{x}_t^{-})^{'}]$, *a priori* covariance of the state estimation error (a priori state covariance),  
$V_{k}^{+} = E[(x_{k} - \hat{x}_t^{+}) \ (x_{k} - \hat{x}_t^{+})^{'}]$, *a posteriori* covariance of the state estimation error,  
$V_{0}^{+} = E[(x_{0}^{+} - \hat{x}_{0}) \ (x_{0}^{+} - \hat{x}_{k})^{'}]$, initial *a posteriori* covariance of the state estimation error (a posteriori state covariance).  
...
$$
\begin{align}
\text{ A posteriori state estimate: } \hat{x}_{k}^{+} &= \hat{x}_{k}^{-} + K_{k}(y_{k} - Z_{k} \ \hat{x}_{k}^{-}) \text{,} && \\
\text{ A posteriori state covariance: } V_{k}^{+} &= (I_{m} - K_{k} \ Z_{k}) V_{k}^{-} && \\  
\text{}
\end{align}
$$
...
Several variations of $\vec{x}_t$ are generated by function *MARSSkf* as Kalman filter and smoother output [@MARSS2020b].  These include *xtT*, $x_{k|K}$, which is the smoothed or offline estimate of the state vector at time *k* given all the time steps in the series *K*; *xtt*, $x_{k|k}$, is the filter estimate of the state vector at time *k* given information up to and including time *k*; and *xtt1*, $x_{k|k-1}$, is the predicted state at time *k* given information up to time *k-1*. The state vector was initialized at k = 0, $\vec{x}_{0}$, to the corresponding parameters estimated in a selected GAM model. Like the state vector, the covariance matrix of the state vector has three estimates in MARSS, $V_{k|K}$, $V_{k|k}$, and $V_{k|k-1}$ depending upon whether smoothed, filtered, or prediction variance are of interests. 
