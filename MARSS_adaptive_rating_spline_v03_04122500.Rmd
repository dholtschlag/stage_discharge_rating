...
Figure `r fig_no + 1` shows the smoothed changes in basis parameters with times of discrete measurements. All parameter series followed a random walk, given that an identity matrix premultiplied $\vec{x}_{t-1}$ in the state equation. The parameter associated with the intercept term, $\beta_0$, generally increases with time. The remaining parameters, $\beta_1 - \beta_4$, tended to take their minimum values at the end of the period of analysis.  At the end of the period of analysis, the maximum correlation of 0.688 occurred between parameters $\beta_0 \ and \ \beta_3$, and the minimum correlation of -0.360 occurred between parameters $\beta_2 \ and \ \beta_4$. The parameter time series are based state-space Model 9.    

```{r, state_parameter_matrix, fig.height = 10, fig.width = 9 }
...
colnames(marss_state_tT) <- c('0', '1', '2', '3', '4', 'Date')
...
marss_state_tT %>% 
  ...
  ggtitle( label = paste0('Figure ', fig_no, '. Time series plot of smoothed estimates of cubic regression spline parameters making up the state vector.') ,
