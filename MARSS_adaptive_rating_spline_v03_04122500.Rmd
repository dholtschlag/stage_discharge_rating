...
In this paper, the process variance Q is a $5\times5$ matrix, corresponding to the dimension of the state vector. The state vector represents the 5 parameters in the initiating GAM model, which is specified by the number of knots. 
took on four general forms named by [@marss2018], which include 'diagonal and equal', 'diagonal and unequal', 'equalvarcov', 'unconstrained', and a custom form enabled by the MARSS software but not explicitly named. In this paper, the special form is referred to as 'std diagonal multiplier', for standardized diagonal multiplier. The diagonal forms are based on the assumption that changes in parameters are independent. Estimation of off-diagonal terms, however, may be more consistent with how changes in parameters occur. The forms of these matrices are illustrated below. 



$$ Q_1 (diagonal \ and \ equal) =  \begin{bmatrix} q & 0 & 0 & 0 & 0 \\ 0 & q & 0 & 0 & 0 \\ 0 & 0 & q & 0 & 0 \\ 0 & 0 & 0 & q & 0 \\ 0 & 0 & 0 & 0 & q\\ \end{bmatrix}$$
Diagonal and Unequal Variances

In the 'diagonal and unequal' form of the Process Covariance Matrix, five parameters are estimated $(q_{11}, q_{22}, \dots,q_{55})$ are estimated.  
...
In usage  The equal variance and covariance (equalvarcov) means that there is one value on the diagonal and one value on the off-diagonal. Thus, the equalvarcov matrix only has two estimated parameters. 

$$ Q_3 (equalvarcov) =  \begin{bmatrix} q & c & c & c & c \\ c & q & c & c & c \\ c & c & q & c & c \\ c & c & c & q & c \\ c & c & c & c & q\\ \end{bmatrix}$$

> Unconstrained Process Covariance Matrix

The 'unconstrained' case for a covariance matrix like Q is restricted to be symmetrical and positive definite, which guarantees invertibility. So, the $5 \times 5$ Q matrix has only 15 unique elements that are estimated rather than 25. 

$$ Q_4 (unconstrained) \ =  \begin{bmatrix} q_{11} & q_{12} & q_{13} & q_{14} & q_{15} \\ q_{12} & q_{22} & q_{32} & q_{42} &  q_{52} \\ q_{13} & q_{23} & q_{33} & q_{43} & q_{53} \\ q_{14} & q_{24} & q_{34} & q_{44} & q_{54} \\ q_{15} & q_{25} & q_{35} & q_{45} & q_{55}\\ \end{bmatrix}$$


> Standardized Diagonal Multiplier for of the Process Covariance Matrix

The standardized diagonal multiplier or 'std diagonal multiplier' form of the process covariance matrix contain five diagonal elements $\hat{q}_{11}, \hat{q}_{22}, \dots,\hat{q}_{55}$ that are based on a related covariance matrix, but standardized so that the elements average 1. A multiplicative constant, $q$ is estimated to multiply the standardized diagonal elements. 
...
>  1 marss_fit_Q_ones_multiplier_R_one_multiplier

MARSS Model with Q = 'diagonal and equal' and R = 'diagonal and equal'
...
> 2 marss_fit_Q_unequal_scalars_R_one_multiplier

MARSS Model with Q 'diagonal and unequal' and R 'diagonal and equal'
...
> 3 marss_fit_Q_ones_multiplier_R_mean_qualifier

MARSS model with Q 'diagonal and equal' and R mean specified variance
...
> 4 marss_fit_Q_unconstrained_R_mean_qualifier

MARSS with Q 'unconstrained' and R mean specified variance of discrete measurements
...
> 5 marss_fit_Q_equalvarcov_R_mean_qualifier

MARSS with Q 'equalvarcov' and R mean specified variance of discrete measurements
...
> 6 marss_fit_Q_covar_multiplier_R_one_multiplier

MARSS model with Q diagonal with covariance multiplier and R 'diagonal and equal'
...
> 7 marss_fit_Q_covar_multiplier_R_mean_qualifier

MARSS model with Q diagonal with covariance multiplier and R mean specified variance of discrete measurements
...
>  8 marss_fit_Q_ones_multiplier_R_series_qualifier

MARSS Model with Q 'diagonal and equal', and R time varying specified measurement variance
...
> 9 marss_fit_Q_covar_multiplier_R_series_qualifier

MARSS Model with Q diagonal with scalar covariance multiplier and R time varying specified measurement variance
