...
The noise vector $\vec{w}_{t-1}$ is assumed to be uncorrelated and have a multivariate-normal distribution (MVN) with mean $\vec{0}$ and process covariance $Q_c$. Although the process covariance is assumed to be time invariant, the magnitudes of each element are dependent on the length of the discrete time step, $\Delta{t}$. In particular, [Simon2006] approximates the variablility as indicated in equation 3 [@Simon2006]. By extention, equations 

$$
\begin{align}
\ Q &\approx \frac{Q_{day}}{\Delta{t}} & \text{where} \Delta{t} \rightarrow 1 \ \text{day, then} \ Q_c \approx Q_{day}
\text{,}\tag{3}\\
\ Q_{hour} &\approx \frac{Q_{day}}{24} \tag{4} \\ 
\ Q_{15-min} &\approx \frac{Q_{hour}}{4} \tag{5} \\ 
\end{align}
$$
