...
# Compute day of year
flow_daily_pub$yday <- yday(flow_daily_pub$Date)

yday_Flow_cd <- as.data.frame( table( as.integer(flow_daily_pub$yday) , flow_daily_pub$Flow_cd), 
                               col.names = c('day_number', 'Flow_cd', 'freq'))

colnames(yday_Flow_cd) <- c('day_number', 'Flow_cd', 'freq') 

yday_Flow_cd$day_number <- as.integer( yday_Flow_cd$day_number )
...
# Methods for Adapting Stage-Flow Ratings and Estimating the Magnitudes and Uncertainties of Streamflow Records
...
# Results of Cubic Regression Splines and State-Space Modeling
...
### Cubic Regression Splines for Rating Periods 18 - 21  
...
marss_state_tt             <- data.frame( t(as.matrix( marss_kfss$xtt, ncol = 5, nrow = 10966)))
...
marss_state$Kalman         <- rep(c('Filter','Smoother' ), each = 10966 )
...
ndx_A  <- which(df_stage_flow_iceDays$Flow_cd == 'A')
...
df_stage_flow_selDays %>% 
  ...
  scale_color_manual( labels =  c('Fair discrete measurement', 'Published daily mean flow',
                                 ...
                                 'Lower 95% smoothed flow',   'Upper 95% smoothed flow',
...
Figure `r fig_no + 1` shows a stacked histogram of the frequency of computed and estimated daily mean flows that were published during the 30-year period of analysis.  Flow is considered computed when unit stage data is available and used with an applicable open-water rating to determine flow. Otherwise, daily mean flow is estimated by other technical means, but is considered less accurate than computed flows. The accuracy of the Kalman smoother estimates were assessed 

The 95-percent confidence interval about the Kalman smoother estimate was used to assess the accuracy of the Kalman smoother magnitude and uncertainty. only computed streamflow values the uncertainty of the published daily flow is a consideration, but not is not quantified. Days within the 30-year period of analysis in    
...
fig_no <- fig_no + 1
flow_daily_pub[ 2:10958,] %>%   
  mutate( wy_day  = hydro.day.new(Date),
          Flow_cd = recode_factor(Flow_cd, 'A'   = 'Computed',
                                           'A e' = 'Estimated',
                                           'P'   = 'Preliminary' )) %>% 
  ...
  theme_bw() +
  scale_fill_manual( labels =  c('Computed', 'Estimated'), values = c('grey', 'red')) +
  geom_vline( xintercept = c(62, 166), color = 'blue', alpha = 0.5) +
...
CRSs developed in a GAM environment provide a basis for developing stage-flow rating relations from data in extended rating tables, or in discrete stage-flow measurement pairs.  In this paper, CRSs developed with five knots closely approximated manually developed rating relations. The CRS parameters (coefficients) and linear basis functions can be used as an initial rating relations for an adaptive rating embedded in a state-space model. Pairs of discrete stage-flow measurements were used 
