...
A state-space model consists of two equations: a state equation, and a measurement equation. The left-hand-side state vector of length *m* equal to the number of knots, $\vec{x}_{t}$ contains unobservable states. In this analysis, the states correspond to time-varying parameters in the CRS at time *t*, $\vec{beta}_t$.  The states are updated from discrete observations in the measurement vector (scalar in this case) $\vec{y}_{t}$. These updates are based on the Kalman gain, computed using error covariance of the state vector, $V$ and the variance of the flow measurement $R$ [@MARSS2020b]. The state and measurement equations are solved alternately for each time step indexed by $t.$ The state-space model is written below:
...
The state equation transitions (or forecasts) the state vector $\vec{x}_t$, of length $m$, from time $t-1$ to time $t$. Generally, a state transition matrix, $B_t$, can be time varying and premultiplies the state vector as: $\vec{x}_t = B_t \ \vec{x}_{t-1}$, where $B_t$ is specified or estimated. In this analysis, $B_t$ is taken as a constant identity matrix, $I_m$, which implies that the state vector is modeled as a random walk. For simplicity, $B$, is omitted in the state equation. 
...
** Summary output for the GAM developed on the basis of extended rating 18 data **

```{r }
summary(gam18_rating)
```
** Summary output for the GAM developed on the basis of extended rating 19 data **

```{r}
summary(gam19_rating)
```

** Summary output for the GAM developed on the basis of extended rating 20 data **
```{r, echo = FALSE}
summary(gam20_rating)

```

** Summary output for the GAM developed on the basis of extended rating 21 data **
```{r echo = FALSE}

summary(gam21_rating)
```

** Summary output for the GAM developed on the basis of discrete measurement pairs for rating period 18 **
```{r echo = FALSE}
summary(gam18_measPair)
```

** Summary output for the GAM developed on the basis of discrete measurement pairs for rating period 19 **
```{r echo = FALSE}
summary(gam19_measPair)
```

** Summary output for the GAM developed on the basis of discrete measurement pairs for rating period 20 **
```{r echo = FALSE}
summary(gam20_measPair)
```

** Summary output for the GAM developed on the basis of discrete measurement pairs for rating period 21 **
```{r echo = FALSE}
