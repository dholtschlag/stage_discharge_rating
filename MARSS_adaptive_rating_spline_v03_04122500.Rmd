...
An adaptive stage-flow estimation approach is developed to track persistent changes in the relation between stage (water-surface elevation) and flow (streamflow discharge) at a streamgage. The approach is initialized by estimating a cubic regression spline for the stage-flow relation within a generalized additive modeling  framework. The initial cubic regression spline parameters can be estimated from data in an existing stage-flow rating table, or from discrete measurements of stage and flow throughout the stage domain of interest. These parameters are applied to a set of basis vectors, derived from the stage values, and used to compute flow for stage-flow relations at an initial time. Changes is the cubic regression spline parameters, reflecting changes in the stage-flow relation, are tracked within a state-space model by use of discrete flow measurements. In addition, the state-space model can be used with Kalman filtering and stage values to compute the magnitude and uncertainty of the streamflow time series. The approach is demonstrated with data from the U.S. Geological Survey streamgage 04122500 Pere Marquette River at Scottville, Mich.  Extension of this approach to track intermittent changes, associated with variable ice-backwater for example, would require a multivariate analysis involving a streamflow network that is beyond the scope of this report.  

# Background

The U.S. Geological Survey (USGS) computes records of streamflow at about 8,500 active streamgages throughout the United States. Streamgages measure outflow from a basin, which is generally topographically defined by a watershed.  Computations are commonly based on continual measurements of stage at 10- to 60-minute intervals. The stage data is used to compute corresponding values of flow based on a empirical relation between stage and flow commonly referred to as a rating curve or table. The rating describes a monotonically increasing relation between stage and flow. A rating is developed and maintained on the basis of discrete, contemporaneous measurements of stage and flow that span the range of stage conditions at the streamgage. Discrete measurements require a field visit to the streamgage, which can be a major element in the cost of streamgage operation.  
...
if( file.exists(site_file)){
  ...
  site_info$station_nm <- "Pere Marquette River At Scottville, MI."
...
This section discusses data required for implementation of the adaptive stage-flow rating approach. Continual streamflow data are commonly computed on the basis of continual stage data, and an empirical relation between stage and flow. The continual intervals are generally less than or equal to an hour in length, and are referred to as unit data.  The relation between stage and flow is based on (periodic) discrete stage and flow measurements obtained during irregular field visits to streamgages. A set of discrete measurements is used to manually define a curvilinear relation between stage and flow over an extended stage range. The stage-flow relation is commonly referred to as a rating curve or table. Discrete measurements continue throughout the period of streamgage operation to confirm, update, extend, and revise the stage-flow relation as needed to accurately compute streamflow. Major changes in the stage-flow relations are indicated by (generally integer) changes in the rating number.   
...
Historically, unit stage and flow values have been considered ancillary data of secondary importance to daily streamflow. High cost of electronic storage of these data were once prohibitive. More recently, however, both unit stage and unit flow data has been stored in electronically accessible formats along with information on discrete stage-flow measurements. Ready access to these data have been instrumental in developing this analysis. 

## Daily Streamflow

Daily mean streamflow data for the Pere Marquette River were obtained for the period of record using the USGS National Water Information System (NWIS) accessed by use of the R package 'dataRetrieval' [@DeCicco2019]. Some of the earlier ancillary data used in computing daily mean flows, however, is only available from archived paper records. In particular, unit stage data and discrete flow measurement data are generally available for Pere Marquette River in electronic format since `r format(date_beg_POA, '%b %d, %Y')`. The most recent data used in the analysis was `r format(date_beg_POA, '%b %d, %Y')`. In this paper, the period of electronic data availability is referred to as the period of analysis (POA). Data summaries presented in the following sections are restricted to this period.
...
A probability density curve can be used to theoretically describe the empirical frequency characteristics of daily flow magnitudes.  In this analysis, the distribution of the natural logarithms of daily flows at Pere Marquette River were closely approximated by the asymmetrical, three-parameter gamma distribution (fig. 2), which is right skewed. Flows ranged from `r flow_hpdi[[1]]` and `r flow_hpdi[[2]]` ft^3^/s, 95-percent of the time.  The minimum and maximum daily flows during this period were `r min(flow_daily_pub$Flow)` and `r max(flow_daily_pub$Flow)`, respectively. The mean flow was 787 ft^3^/s. Thus, a natural logarithm transformation was applied when modeling these flows.     
...
Discrete flow measurement data for Pere Marquette River occurring during the period of analysis were retrieved from the USGS National Water Information System (NWIS) using the R package dataRetrieval [@DeCicco2019]. Table `r tab_no + 1` summarizes the type of flow control and the accuracy of discrete flow measurements provided by the flow qualifier. The most common control conditions were a 'Clear Channel', and the most common measurement flow accuracy qualifier was 'Good.'  
...
A stage-flow relation is adapted by changing cubic regression spline (CRS) parameters on the basis of discrete measurements of stage and flow.  Intervening measurements of stage, however, can be used to generate a set of basis functions (one for each knot) from an underlying CRS and used to compute the magnitudes and uncertainties of daily or unit streamflow. 

The time series of computed daily mean stage values (fig. `r fig_no + 1`) shows a seasonal variation within which annual minimums appear to have a downward trend. In particular, during the early 1990s, minimum annual stage values were generally greater than 1.6 ft above gage datum, while during the 2010s, minimum annual stage values were generally 0.6 ft lower. This pattern is consistent with the possible trend in the discrete measurements of stage data discussed earlier. Based on visual inspection of the time series, the variability of stage values may be increasing with time.
...
### Selected Stage-Flow Ratings with Discrete Measurements
...
A CRS can be used to approximate either an existing rating from an extended rating table, or to develop a new rating from a set of discrete stage-flow measurements representing unobstructed (typical) flow conditions at a streamgage. A generalized additive modeling (GAM) framework[@Wood2006] provides a basis for assessing the statistical significance of individual CRS parameters and provides measures of the accuracy of the CRS model.  Parameters of the CRS, $\vec{\beta}$, form the state vector of the state equation, which together with the measurement equation form the state-space model. In this analysis, estimates of the process covariance $Q$ and measurement variance $R$ parameters in the state-space model are based at daily time steps. In applications, unit time steps could be used with adjustments to the $Q$ parameters in the state equation that are proportional to changes in the length of the time intervals. 

The uncertainty of the state vector, as indicated by the state covariance matrix $V$, increases with time between measurements based on an estimated process covariance matrix $Q$ in the state equation. Within the measurement equation, the state vector is premultiplied by a set of basis functions with compatible length to compute unit or daily flows. On days when a discrete measurement occurs, the CRS parameters are updated to adapt the rating curve to possible changes in the stage-flow relation. The updates reduce the differences between the estimated and measured flows, and reduce the uncertainty of the state covariance matrix $V$.  

The remaining discrepancies between discrete measurements of flow and estimated flow, both in natural logarithmic units, are described by the measurement variance $R$ in the measurement equation. The value of $R$ can be specified or estimated. Once, $Q$ and $R$ are obtained, a set of CRS parameters can be computed that sequentially adapt the rating curve based on each discrete flow measurement used in the analysis. In addition, a Kalman filter [@Simon2006] can be applied to estimate the magnitude and uncertainty of daily or unit flows based on daily or unit stages. Details of this process are described in the following paragraphs. 

## Cubic Regression Splines

CRS are piecewise polynomials with constraints to ensure continuity to the second derivative at interior knots and zero second derivatives at the boundary knots [@Wood2006].  Individual polynomials are cubic (third order). Special constraints on the boundary knots ensures linear extrapolation beyond the domain of the stage data. CRS parameters were estimated within a GAM to flexibly represent the monotonically increasing relation between stage and flow. The GAM package 'mgcv' [@mgcv2019] provided parameter estimation, statistics on model fit, and utility functions like *gam()*, *matrix.model()* and *predict()* for developing the CRSs within the R [@cite_R] programming environment. 

A simple cubic polynomial equation can be specified by a set of parameters $\beta_j$ and polynomial terms (in place of basis functions) $x^j$ for $j = 0, 1, 2, 3$, such that $\hat{y}_i = \beta_0 + x_i \beta_1 + x_i^2 \beta_2 + x_i^3 \beta_3.$ Although a cubic polynomial is often a useful approximation for point estimation, cubic splines are preferred when estimating the response over a range of values [@Wood2017]. 

CRSs used in this analysis were specified as having 5 knots, which was thought to be sufficiently flexible for the stage-flow relations at the selected streamgage. Five knots resulted in two boundary knots at the minimum and maximum of the stage data, and three interior knots at the 25^th^-, 50^th^-, and 75^th^-percentiles. Generally, the *gam()* function defaults to placing the interior knots at evenly-spaced quantiles of the data, although the knot locations can be specified to coincide with apparent breaks in the curvature of the rating. The basis functions themselves are not affected by the distribution of the response variable (flow).  
...
where $b_j(stage_i)$ refers to the j^th^ basis function, $\varepsilon_i$ is the residual from the model estimate.  
...
In this analysis, the state vector represents the five time-varying parameters in the cubic regression spline describing stage-flow relations. A logarithmic transformation is applied to flow before fitting the CRS. Initial values for the state vector, $\vec{x}_{0}$ were based the five parameters estimated for a cubic regression spline developed using extended rating 18 (Appendix 1).    

The zero-mean uncorrelated time series $w_{t-1}$ has a multivariate normal (*MVN*) distribution with process covariance $Q_c$, and is considered to be time invariant. $Q_c$ represents the process covariance as the length of the time steps, $\Delta{t}$, tends to zero. The time series $w_t$ is an unbiased (zero mean) measurement error component with variance $R_t$. The variance of $R_t$ can be measured or specified.  

The components of the state vector time series are not directly observable, but are estimated by use of the Kalman filter [@Simon2006]. The process covariance matrix and measurement variance series can be estimated by a maximum-likelihood algorithm for parameter estimation based on the Expectation-Maximization (EM) algorithm [@Holmes2013]. In addition, parameters of $Q$ and $R$ can also be estimated more quickly by a quasi-Newton method by use of the BFGS algorithm with the R function *optim()*.  Values in $R$ can also be specified based on a perceived percentage error associated with qualifiers specified by hydrographers making the flow measurement. 


The state equation transitions (or forecasts) the state vector $\vec{x}_t$, of length $m$, from time $t-1$ to time $t$. Generally, a state transition matrix, $B$, can be time varying and premultiplies the state vector as: $\vec{x}_t = B \ \vec{x}_{t-1}$, where $B$ is specified or estimated. In this analysis, $B_t$ is taken as a constant identity matrix, $I_m$, which implies that the state vector is modeled as a random walk. For simplicity, $B$, is omitted in the state equation. 


#### State Vector Time Series $\vec{x}_t$

$\vec{x}_t$ is the time series of state vectors, where each vector has a length of corresponding to the number of knots specified for the CRS.  The state-vector is updated only at times (on days) of discrete flow measurements. If the measurement vector contains a missing value indicator, the measurement update is skipped and the state vector is unchanged. The covariance of the state vector before the measurement update $V_t^-$, however, is incremented by $Q$ at each time step in the state equation, while  $V_t^+$ is state variance updated (reduced) only when discrete flow measurements are indicated in the $y_t$ vector.

#### Process Covariance Matrix $Q$

In a continuous-time representation of the discrete state-space equations [1 and 2], $\vec{w}_{t}$ is an uncorrelated time series vector defined at infinitesimal intervals of $Delta{t}$. The $\vec{w}$ has a multivariate-normal distribution (MVN) distribution with a mean equal to $\vec{0}$ and process covariance $Q_c$.  In transitioning to a discrete process covariances $Q_d$, where $\Delta{t}$ is a defined positive value, [@Simon2006] provides the approximation shown in equation [3], where   

$$
\begin{align}
\ Q_c &\approx \frac{Q_d}{\Delta{t}} \text{,}\tag{3}\\
\text{where } Q_c  &= \text{ is the continuous-time process covariance,} \\
Q_d &= \text{ is the discrete-time process covariance, } \\
\ \Delta{t} &= \text{the length of the discrete time step.}\\ 
\end{align}
$$

In this analysis, parameters for $Q_d$ were estimated at daily time steps for the 10,957 days within the period of analysis based on 222 intervening discrete measurements. This strategy avoided the need to estimate  referenced as $Q_{day}$.  
It follows from equation [3] that discrete process defined  $Q_{hour} \approx Q_{day}/24$, or that $Q_{15-min} \approx Q_{hour}/4$ would be appropriate for simulation of unit streamflow data given that $\Delta{t}$ is infinitesimal for $Q_c$.  This implies that elements of $Q_{day}$, derived from parameter estimation in MARSS, can be divided by the number of unit intervals in a day, to simulate flow magnitudes and uncertainties based on unit stage values. 


The process covariance matrix $Q$ increases the uncertainty of the state vector $\vec{x}_t$, which is characterized in the time-varying state covariance matrix $V_t$ [@simon2006].  


The process covariance reflects the increase in uncertainty with the forecast from time $t-1$ to time $t$. According to [@MARSS2020a], the maximum likelihood estimates of $Q$ and $R$ are fundamentally biased, and this bias is more severe when either $Q$ or $R$ are small. This bias arises because variance is constrained to be positive, and this variance does not go to zero with increasing sample size. So that $Q$ was as large as practical while accurately identifying the timing of discrete measurements to the day, a daily time step was used for the estimation of $Q$. This discretization is thought to be adequate for describing the uncertainty of parameters of the rating curve. When estimating the uncertainty of unit flow magnitudes, however, linearity would be applied so that the daily $Q$ value would be divided by the number of unit time steps per day to better approximate the increase in variance of flow at unit time intervals.  
...
Daily time steps were used in this paper because the most frequent time between discrete flow measurements was 42 days. On average, therefore, there is one measurement update for each 42 state updates in the alternating solution of the state space model. Estimating state-space model parameters at unit intervals would increase the computational burden of solving the state equation by one or two orders of magnitude using hour or 15-minute time steps were used. In addition, the estimation results may be degraded. In particular, [@MARSS2020b] note that when $Q$ (or $R$) gets very small, the bias in the maximum-likelihood estimates of variances becomes more severe. The magnitude of $Q$ is generally small ($<10^{-05}$) even at daily time steps. Alternatively, one might estimate $Q$ at daily time steps and divide $Q$ by the number of time steps in a day for estimation of streamflow records. 

In this paper, the process covariance $Q$ is a $5\times5$ matrix, corresponding to the dimension of the state vector. The state vector represents the 5 parameters in the initiating CRS model. The covariance matrices took on four general forms that can be defined by special tags defined within the MARSS software [@MARSS2020b]. These tags include 'diagonal and equal', 'diagonal and unequal', 'equalvarcov', 'unconstrained,' in which the tag provides a shortcut string to define a particular form of the matrix. For example, in the model specification, the process covariance matrix can be explicitly defined as: 'Q = 'diagonal and equal' or explicitly, as discussed below for the 'std diagonal multiplier' form for standardized diagonal multiplier. The diagonal forms are based on the assumption that changes in parameter values are independent. Estimation of off-diagonal terms, however, may be more consistent with covariance effects among parameters, and some of these forms are estimated and evaluated. The forms of these matrices are illustrated and discussed below. 
...
The design matrix ${Z}_{t}$ is a $10957\times 5$ array series of vectors that form the design matrix. This matrix contains elements of the basis vectors computed from stage data on all days during the period of analysis. The stage data was computed as daily means of unit value stage data, except on days of discrete measurements. On days of discrete measurements, the discrete stage measurement themselves were substituted for the daily mean stage to more precisely correspond to the discrete flow measurements.     


### The Kalman Filter  

The Kalman filter [@Simon2006] is used to propagate changes in the state vector in response to measurement data within the state-space model. A short discussion of the Kalman filter, with applicable modifications of notation, is included below to provide some intuition for the interested reader. To simplify the notation, it is assumed that the state is a vector, $\hat{x} = x$,  


**Definitions and Initial Conditions**  
$\hat{x}_{t}^{-} =  E[x_{t}|y_1, y_2, ..., y_{t-1}]$, *a priori* state estimate at time *t*,  
$\hat{x}_{t}^{+} =  E[x_{t}|y_1, y_2, ..., y_{t}]$, *a posteriori* state estimate at time *t*,  
$\hat{x}_{0}^{+} = E[x_{0}]$, initial *a posteriori* state estimate at time = 0,  
$V_{t}^{-} = E[(x_{t} - \hat{x}_t^{-}) \ (x_{t} - \hat{x}_t^{-})^{'}]$, *a priori* covariance of the state estimation error (a priori state covariance),  
$V_{t}^{+} = E[(x_{t} - \hat{x}_t^{+}) \ (x_{t} - \hat{x}_t^{+})^{'}]$, *a posteriori* covariance of the state estimation error,  
$V_{0}^{+} = E[(x_{0}^{+} - \hat{x}_{0}) \ (x_{0}^{+} - \hat{x}_{t})^{'}]$, initial *a posteriori* covariance of the state estimation error (a posteriori state covariance).  
...
$$
\begin{align}
\text{ A priori state covariance: } V_{t}^{-}  &= B \ V_{t-1}^{+} \ B^{'} + Q_{t-1} \text{,} && \\  
\text{ Kalman gain: } K_{t} &= V_{t}^{+} \ Z_{t}^{'} \ R_{t}^{-1} \text{,} &&  \\  
\text{ A priori state estimate: } \hat{x}_{t}^{-} &= B \ x_{t-1}^{+} \text{,} && \\  
\end{align}
$$
    Skip measurement update if $y_{t}$ = NA and increment *t* if t < max(t) = T. 
$$
\begin{align}
\text{ A posteriori state estimate: } \hat{x}_{t}^{+} &= \hat{x}_{t}^{-} + K_{t}(y_{t} - Z_{t} \ \hat{x}_{t}^{-}) \text{,} && \\
\text{ A posteriori state covariance: } V_{t}^{+} &= (I_{m} - K_{t} \ Z_{t}) V_{t}^{-} && \\  
\text{}
\end{align}
$$
    Increment *t* if t < max(t) = T.     
***  
...
|:  Zt                 |: Zt is the design matrix formed by the daily time series of basis functions. The basis functions are derived from the CRS based for the GAM rating 18 computed using the rating table. Daily values for the basis function were predicted using daily mean stage, except on days with discrete measurements. On these days, the discrete measured stage was substituted for the daily mean value.                                                                                         |  
...
### Basis Functions for Cubic Regression Splines

Basis functions for CRS were developed from extended rating table data to show the shape of these smooth functions, and how the curves shifted between rating periods. Figure `r fig_no + 1` shows basis functions 1 through 4 and corresponding knot locations as vertical lines. Each basis function has an average value of zero over the stage range, except for basis 0 (not plotted), which has a constant value of one for all stages. The maximum of each basis function occurs near the corresponding knot location. Note that the horizontal distances between knots (vertical lines) and the vertical distances between basis curves decreases with stage and time between rating periods until all curves converge to a point at basis 4.

The basis functions provide the design matrix for estimating flow in the CRS and in the state-space measurement equation.  The function *matrix.model* can be used to extract the basis functions for all elements in the corresponding rating table.  The function *predict* along with the argument 'type = lpmatrix' is used with with the *newdata* argument to provide basis functions from daily stage values.   
...
# marss_state_tt is the estimate of the state at time t given up to time t
...
df_stage_flow_selDays %>% 
  ...
  # ggplot(   data   = df_stage_flow_selDays, aes(x = Date  )) +
  # geom_line(data   = df_stage_flow_selDays, aes( y = exp(ytT) ),   color = 'red' ) +
  # geom_line(data   = df_stage_flow_selDays, aes( y = exp(cl025) ), color = 'blue', size = 0.5) +
  # geom_line(data   = df_stage_flow_selDays, aes( y = exp(cl975) ), color = 'blue', size = 0.5) +
  # geom_line(data   = df_stage_flow_selDays, aes( y = flow_daily_pub ), color = 'darkgreen', size = 0.7) +
  # geom_ribbon(data = df_stage_flow_selDays, aes(ymin = y_ice, ymax = exp(ytT) ), fill = 'grey', alpha = 0.5  ) +
  # theme_bw() +
  # scale_y_log10() +
  # geom_point(data = df_stage_flow_meaDays, aes( y = flow_discrete_mea,   color = qualifier), size = 1.5 ) +
  # geom_point(data = df_stage_flow_meaDays, aes( y = flow_discrete_mea ), size = 5, shape = 1, color = 'black'  ) +
...
Stage and flow data from the U.S. Geological Survey streamgage 04122500 Pere Marquette River at Scottville, Michigan, were used to develop a methodology to automatically adapt a stage-flow rating to persistent changes in the relation.  Electronically accessible unit stage, published daily mean flow, discrete measurements pairs of stage and flow, and extended rating table data for ratings 18, 19, 20, and 21 were utilized to track changes in the stage-flow relation across the 30-year period of analysis from Oct. 1, 1989 to September 30, 2019. 

Cubic regression splines (CRSs) estimated in a generalized additive modeling (GAM) framework provided a basis for developing a set of stage-flow ratings from data in extended rating tables, and from data in sets of discrete stage-flow measurement pairs. A set of five parameter (five knot) CRSs was developed with corresponding static linear basis functions, determined from stage values. The CRSs provided equations for predicting flow at these initial conditions. Parameters from the CRS developed by use of the extended rating table 18 was selected to represent initial rating conditions. 

With persistent change, however, initial stage-flow ratings become biased estimators of flow over time. This bias can be detected on the basis of continuing discrete measurements of stage and corresponding flow.  A strategy was developed to adapt ratings to persistent changes by making the parameters of the CRS equation time varying, but leaving the basis functions unchanged. 

To implement this strategy, a state-space model, which contains a state equation and a measurement equation, was utilized. The state and measurement equations are solved alternately for each time step in the analysis.  

The CRS parameter vector formed the state vector in the state equation. The state equation provides a forecast of the state vector for the next time step by characterizing random temporal disturbances in the state (parameter) vector, which is quantified by an estimated process covariance matrix $Q$. After the temporal update, the state vector is premultiplied by basis functions in measurement equation, to match a discrete flow measurement, if one is available, on that time step. Difference between the predicted and measured flow, in logarithm units, provides a basis for estimating the measurement variance $R$, unless this uncertainty is specified.   

Nine state-space models were estimated for different forms for the process covariance matrix and the measurement variance. A state-space model with an equal and diagonal  

In this paper, CRSs developed with five knots, and five corresponding parameters, closely approximated manually developed rating relations. 

The CRS parameters were made time varying to adapt the initial rating to persistent changes in the stage-flow relation using discrete measurements within a state-space modeling framework.




Pairs of discrete stage-flow measurements were used 



CRSs were developed in a GAM environment to estimate stage-flow relations during rating periods 18 through 21 for USGS streamgage 04122500 Pere Marquette River at Scottville, Mich.  Regressions for all rating periods were developed using stage-flow data from expanded (0.01 ft increments of stage) rating tables, and from pairs of discrete stage and flow measurements obtained during the corresponding rating period.  The curves formed by the CRSs and the rating tables matched closely as indicated by a coefficient of determination (r^2^ value) of approximately 1. The indicated uncertainties of these CRSs curves, however, were uninterpretable because the uncertainty measures were heavily influenced by the arbitrary discretization interval (0.01 ft) for the stage data. Still, the parameters of these rating curves may have utility in automatically adapting existing stage-flow relations developed manually to on-going stage-flow changes with subsequent discrete measurement data.  

CRSs developed within a GAM framework provided a flexible and accurate method for estimating the stage-flow relation at a streamgage either directly from an expanded stage-flow rating table, or from discrete pairs of stage-flow measurements. The GAM environment produced a set of parameter estimates for the CRSs and a method for generating basis vectors from stage data.  The CRS parameters were used as the state vector in the state equation of a state-space model. The set basis vectors formed a design matrix that premultiplied the state vector in the measurement equation to compute flow.  The process covariance matrix $Q$ in the state equation and measurement variance $R$ in the measurement equation were estimated by use of discrete measurements of stage and flow.  
