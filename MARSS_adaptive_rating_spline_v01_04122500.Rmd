...
## Methods for Developing an Adaptive Stage-Flow Rating
...
# Print table of starting and ending dates for selected rating periods
rating_history %>% 
...
table( discrete_meas$control_type, discrete_meas$qualifier ) %>%
  kable( digits = 4, 
         caption = paste0('Table ', tab_no, ',. Frequency of flow measurements by control type and measurement qualifiers at ',
...
df %>% 
  ...
  labs( title = paste('Figure', fig_no, '. Distribution of days between consecutive flow measurements at \n04122500 Pere River at Scottville, MI from', date_beg_model, 'to', date_end_model) ) +
...
```{r set_marss_local_QdiagEqual_RdiagEqual}
...
```{r set_marss_local_QdiagEqual_RdiagSpecify}
...
```{r set_marss_local_QdiagMulti_Rseries}


if( file.exists( 'marss_fit_QdiagMulti_Rseries.rds' )){
  marss_fit_QdiagMulti_Rseries <- read_rds( 'marss_fit_QdiagMulti_Rseries.rds' )
} else {
  ...
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagMulti_Rseries  <- MARSS(Yt, model = model.gen, control = list(maxit =  1000, trace = 1, 
                                                            conv.test.slope.tol = 0.05),
                      inits = coef(marss_fit_QdiagMulti_Rseries))
  
  write_rds(marss_fit_QdiagMulti_Rseries, path = 'marss_fit_QdiagMulti_Rseries.rds')
}
...
#   R is based on teh perceived measurement quality, in percent
marss_fit_QdiagMulti_Rseries     <- read_rds('marss_fit_QdiagMulti_Rseries.rds')


# Set up data.frame to compare models
marss_compare <- data.frame( Model = c('Q diagonal and equal, and R scalars estimated', 
                                       'Q diagonal and equal, R mean specified',
                                       'Q diagonal and equal, R series specified', 
                                       'Q diagonal unequal multiplier, R series specified'), 
                   'Method'         = c( 'BFGS', 'BFGS', 'kem', 'kem' ),
                   'Q'              = format( c(marss_fit_QdiagEqual_RdiagEqual$par$Q,
                                                marss_fit_QdiagEqual_RdiagSpecify$par$Q,
                                                marss_fit_QdiagEqual_Rseries$par$Q,
                                                marss_fit_QdiagMulti_Rseries$par$Q), digits = 4),
                   'R'              = c(format(marss_fit_QdiagEqual_RdiagEqual$par$R, digits = 4),
                                              'Mean_Qualifier', 'Meas_Qualifier', 'Meas_Qualifier'),
                   'sqrt_R'         = c(format(sqrt(marss_fit_QdiagEqual_RdiagEqual$par$R), digits = 4),
                                        format(sqrt(marss_fit_QdiagEqual_RdiagSpecify$model$fixed$R), digits = 4),
                                        format(sqrt(mean(marss_fit_QdiagEqual_Rseries$marss$fixed$R[1,1,][ndx_ian])), digits = 4),
                                        format(sqrt(mean(marss_fit_QdiagMulti_Rseries$marss$fixed$R[1,1,][ndx_ian])), digits = 4)),
                   'likelihood'     = format(c(marss_fit_QdiagEqual_RdiagEqual$logLik, 
                                               ...
                                               marss_fit_QdiagMulti_Rseries$logLik), digits = 4),
                   'AICc'           = format(c(marss_fit_QdiagEqual_RdiagEqual$AICc,
                                               marss_fit_QdiagEqual_RdiagSpecify$AIC,
                                               marss_fit_QdiagEqual_Rseries$AICc,
                                               marss_fit_QdiagMulti_Rseries$AICc), digits = 4))
...
marss_compare %>% 
  ...
  add_header_above( c(' ', 'Parameter\nEstimation' = 1, 'Process\nVariance' = 1, 
  ...
  footnote(general = c('likelihood, which is maximized in model selection,',
           ...
           'BFGS: Broyden–Fletcher–Goldfarb–Shanno quasi Newton parameter estimation',
           'kem:  kernelized expectation-maximization'))
...
# marss_fit   <- marss_fit_QdiagMulti_Rseries
