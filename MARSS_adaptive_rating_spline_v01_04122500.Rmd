...
# Zt <- t(predict( gam21, newdata = data.frame( stage_discrete_mea = 
...
if ( file.exists( 'marss_fit_QdiagEqual_Rseries.rds' )){
  ...
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagEqual_Rseries  <- MARSS(Yt, model = model.gen, method = 'kem',
                                         control = list(maxit =  1000, trace = 1, conv.test.slope.tol = 0.05),
                                         inits = coef(marss_fit_QdiagEqual_Rseries))
  write_rds(marss_fit, path = 'marss_fit_QdiagEqual_Rseries.rds')
}
...
design_matrix_rating <- predict( gam18_rating,  newdata = data.frame( stage = rating18$stage), 
                                 type = 'lpmatrix')

# Design matrix for rating augmented with corresponding stage
stage_design_matrix_rating <- cbind(rating18$stage, design_matrix_rating) 
...
flow_rating_DFM <- matrix(NA, nrow = nrow(rating18), ncol = length( ndx_DFM ) )

# design_matrix <- as.matrix(design_matrix_rating[,1:5])

for (i in 1:length(ndx_DFM )){
  flow_rating_DFM[ i ] <- exp( design_matrix_rating[i,] %*% matrix( marss_kf$xtT[ , ndx_DFM[ i ]], 5, 1 ) )
}

# design_matrix_rating_vec <- matrix( flow_xtT, ncol = 1, byrow = FALSE)

tmp_df <- data.frame( Date       = rep(df_stage_flow_allDays$Date[ ndx_DFM ], each  = nrow(   rating18)),
                      stage      = rep(rating21$stage,                        times = length( ndx_DFM )),
                      marss_flow = design_matrix_rating_vec )
...
# Setting up a matrix to compute the adaptive rating curve for all days of flow measurement
# flow_xtT <- 
adapt_rating <- matrix(NA, nrow = nrow(rating18), ncol = length( ndx_allDays_measDays ) )


# design_matrix_rating_aug_matrix <- as.matrix(design_matrix_rating_aug[,2:6])

for (i in 1:length( ndx_DFM )){
  adapt_rating[ , i ] <- exp( design_matrix_rating %*% matrix(marss_kf$xtT[ 1:5, ndx_allDays_measDays[ i ]], 5, 1) )
}
...
tmp_df %>% 
  ...
  geom_line(  aes( x = marss_flow, y = stage), size = 0.5, color = 'blue', alpha = 0.5) +
