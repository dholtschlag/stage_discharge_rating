...
## Model kem_fit_QdiagEqual_RdiagEqual

```{r set_marss_local_QdiagEqual_RdiagEqual}

# Specify Q and R matrix characteristics
Q = 'diagonal and equal'
R = 'diagonal and equal'

#                     kem_fit <- read_rds('kem_fit_QdiagEqual_RdiagEqual.rds')
# kem_fit_QdiagEqual_RdiagEqual <- read_rds('kem_fit_Qdiag_Rscalar.rds')
...
marss_fit_QdiagEqual_RdiagEqual <- read_rds('kem_fit_QdiagEqual_RdiagEqual.rds')

# Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
marss_fit  <- MARSS(Yt, model = model.gen, control = list(maxit =  1000, trace = 1), 
                 method = 'BFGS',
                 inits = coef(marss_fit))


write_rds(marss_fit, path = 'marss_fit_QdiagEqual_RdiagEqual.rds')


marss_kfss <- MARSSkf( marss_fit )
...
#                     kem_fit <- read_rds('kem_fit_QdiagEqual_RdiagEqual.rds')
# kem_fit_QdiagEqual_RdiagEqual <- read_rds('kem_fit_Qdiag_Rscalar.rds')
# write_rds(kem_fit_QdiagEqual_RdiagEqual, path = 'kem_fit_QdiagEqual_RdiagEqual.rds')
# kem_fit <- read_rds('kem_fit_Qdiag_RSeries.rds')

# Specify model
model.gen=list(Z=Zt,A=A,R=R,U=U,Q=Q,x0=X0,V0=V0,tinitx=0)

# Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
marss_fit  <- MARSS(Yt, model = model.gen, method = 'kem',
                  control = list(maxit =  100, trace = 1, conv.test.slope.tol = 0.05),
                 inits = coef(marss_fit))
...
# State variance matrix is based on the gam18_dataPair$Vp
Q <- matrix(list('0.000128978*q',0,0,0,0,
                 0,'0.000666674*q',0,0,0  ,
                 0,0,'0.0004653482*q',0,0,
                 0,0,0,'0.0008095793*q',0,
...
marss_fit_QdiagMulti_Rseries <- read_rds('kem_fig_1g.rds')
...
marss_fit  <- MARSS(Yt, model = model.gen, method = 'kem', 
                       control = list(maxit =  200, trace = 1, conv.test.slope.tol = 0.05), 
                 inits = coef(marss_fit))

# saveRDS(kem_fit_test, file = 'kem_fit_QdiagMulti_Rseries.rds')
...
design_matrix_allDays <- predict( gam21_dataPair, newdata = data.frame( stage_discrete_mea = 
...
design_matrix_rating <- predict( gam21_dataPair,  newdata = data.frame( stage_discrete_mea = rating21$stage), 
                                 type = 'lpmatrix')
...
Z <- predict( gam21_dataPair, newdata = data.frame( stage_discrete_mea = rating21$stage ),
              type = 'lpmatrix')
