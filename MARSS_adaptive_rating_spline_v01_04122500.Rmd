---
title: "Adaptive Stage-Discharge Rating Estimation using a Regression Spline Embedded in a Kalman Filter"
...
```
Streamgage `r site_info$site_no` `site_info$station_nm` has a drainage area of `r site_info$ 



```{r register_key, echo = FALSE, fig.height = 7, fig.width = 7}
...
map <- read_rds(path = "C:/Home/projects/spline_rating_curves/stage_discharge_rating/maps/site_map")

watershed    <- st_read("C:/Home/ws_012/fhwa_flood_regions/gis/shapefiles/04122500.shp")
...
map %>% 
  ...
  ggtitle( paste0('Figure ', fig_no, '. U.S. Geological Survey streamgage 04122500 Pere Marquette River at Scottville, Mich. and drainage basin'))
...
gam_stats %>% 
  kable( caption = paste('Table', tab_no, '. Summary statistics of five-knot cubic regression splines from Genearlized Additive Models for selected rating periods 18 - 20 and the Period of Analysis (POA).' )) %>% 
  kable_styling() %>% 
