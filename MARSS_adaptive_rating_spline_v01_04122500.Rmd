...
This paper develops an automated approach for tracking persistent changes in stage-flow relations using on-going discrete-flow measurements and continually updated stage data. Given that discrete flow measurement commonly occur at about 6-week intervals, persistent changes must exist for several months to be detected by the proposed methodology. Tracking intermittent changes may benefit from analysis of multiple streamgages in a network, but is beyond the scope of this study. 
...
> Probability Distribution Daily Flows


Daily mean flows at Pere Marquette River during the period of analysis varied from `r flow_hpdi[[1]]` and `r flow_hpdi[[2]]`, 95-percent of the time.  The minimum and maximum daily flows during this period were `r min(flow_daily_pub$Flow)` and `r max(flow_daily_pub$Flow)`, respectively. The natural logarithm of flows were consistent with a three parameter gamma distribution (fig. 2). 
...
flow_daily_pub %>% 
  ...
  # geom_vline( xintercept =  min(flow_daily_pub$Flow)) +
  geom_line( data = gam3_df, aes( x = gam3_seq, y = gam3_den), color = 'red' ) +
  xlab('Natural log of Flow, in cubic feet per second') +
  ylab('Probability density') +
  annotate('text', x = 7.40, y = 1.15, label = 'Theoretical Density: Gamma Distribution ', color = 'red', size = 3.8) +
  annotate('text', x = 7.65, y = 1.09, label = paste0('dgamma3( shape = ', format( gam3_parm$estimate[1], digits = 4),
                                    ', scale = ', format( gam3_parm$estimate[2], digits = 4),
                                    ', thres = ', format( gam3_parm$estimate[3], digits = 4),' )'), color = 'red', size = 3.8) +
  annotate('segment', x = 6.54, y = 1.13, xend = 6.675, yend = 1.15, color = 'red' ) +
  annotate('text', x = 6.5, y = 0.3, label = 'Empirical Density' ) +
...
df %>% 
  ...
  annotate('text', x = 52, y = 25.5, label = 'Mean', hjust = -1, vjust = 0, color = 'blue')
...
tmp %>% 
  kable( caption = paste0('Table ', tab_no, ' Number of discrete flow measurements during selected rating periods '),
         col.names = c('Rating number', 'Starting date', 'Ending date', 'Number of Measurements')) %>% 
...
> Plot Historical Stage-Flow Ratings
...
gam18_summary  <- summary(gam18_measPair)
gam19_summary  <- summary(gam19_measPair)
gam20_summary  <- summary(gam20_measPair)
gam21_summary  <- summary(gam21_measPair)
gamPOA_summary <- summary(gamPOA_measPair)
...
gam_stats <- data.frame('Model' = c('gam18', 'gam19', 'gam20', 'gam21', 'gamPOA'),
                           ...
                           'Intercept'      = format(c( gam18_measPair$coefficients[1], gam19_measPair$coefficients[1],
                           gam20_measPair$coefficients[1], gam21_measPair$coefficients[1], gamPOA_measPair$coefficients[1]),
                           digits = 4),
                           'beta1'      = format(c( gam18_measPair$coefficients[2], gam19_measPair$coefficients[2],
                           gam20_measPair$coefficients[2], gam21_measPair$coefficients[2], gamPOA_measPair$coefficients[2]),
                           digits = 4),
                           'beta2'      = format(c( gam18_measPair$coefficients[3], gam19_measPair$coefficients[3],
                           gam20_measPair$coefficients[3], gam21_measPair$coefficients[3], gamPOA_measPair$coefficients[3]),
                           digits = 4),
                           'beta3'      = format(c( gam18_measPair$coefficients[4], gam19_measPair$coefficients[4],
                           gam20_measPair$coefficients[4], gam21_measPair$coefficients[4], gamPOA_measPair$coefficients[4]),
                           digits = 4),
                           'beta4'      = format(c( gam18_measPair$coefficients[5], gam19_measPair$coefficients[5],
                           gam20_measPair$coefficients[5], gam21_measPair$coefficients[5], gamPOA_measPair$coefficients[5]),
                           digits = 4),
...
basis18 <- predict( gam18_measPair, newdata = data.frame( stage_discrete_mea = rating21$stage ),
                                           type = 'lpmatrix')
...
basis19 <- predict( gam19_measPair, newdata = data.frame( stage_discrete_mea = rating21$stage ),
                                           type = 'lpmatrix')
...
basis20 <- predict( gam20_measPair, newdata = data.frame( stage_discrete_mea = rating21$stage ),
                                           type = 'lpmatrix')
...
basis21 <- predict( gam21_measPair, newdata = data.frame( stage_discrete_mea = rating21$stage ),
                                           type = 'lpmatrix')
...
basisPOA   <- predict( gamPOA_measPair, newdata = data.frame( stage_discrete_mea = rating21$stage ),
                                           type = 'lpmatrix')
...
Q  is the fixed 10X10 covariance matrix of parameters
...
## Model marss_fit_QdiagEqual_RdiagEqual

```{r marss_QdiagEqual_RdiagEqual}

# Specify model
if( file.exists('marss_fit_QdiagEqual_RdiagEqual.rds') ){
  marss_fit_QdiagEqual_RdiagEqual <- read_rds('marss_fit_QdiagEqual_RdiagEqual.rds')
} else{
  ...
  marss_fit_QdiagEqual_RdiagEqual  <- MARSS(Yt, model = model.gen, control = list(maxit =  5000, trace = 1), 
                 method = 'BFGS',
                 inits = coef(marss_fit_QdiagEqual_RdiagEqual))
  write_rds(marss_fit_QdiagEqual_RdiagEqual, path = 'marss_fit_QdiagEqual_RdiagEqual.rds')
}
...
```{r marss_QdiagEqual_RdiagSpecify}

# Specify model
if( file.exists('marss_fit_QdiagEqual_RdiagSpecify.rds') ){
  marss_fit_QdiagEqual_RdiagSpecify <- read_rds('marss_fit_QdiagEqual_RdiagSpecify.rds')
} else{
  Q <- 'diagonal and equal'
  # R is specified by qualifiers of discrete flow measurement
  R  <- matrix(mean( (df_stage_flow_allDays$se_pct[ndx_ian_Yt]/100)^2),1,1)
  model.gen=list(Z=Zt,A=A,R=R,U=U,Q=Q,x0=X0,V0=V0,tinitx=0)
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagEqual_RdiagSpecify  <- MARSS(Yt, model = model.gen, control = list(maxit =  1000, trace = 1), 
                 method = 'BFGS')
  # Re-estimation with method 'kem' produces the same Q.diag parameter
  # marss_fit_QdiagEqual_RdiagSpecify  <- MARSS(Yt, model = model.gen, 
  #                                               control = list(maxit =  1000, trace = 1, conv.test.slope.tol = 0.05), 
  #                method = 'kem', inits = coef(marss_fit_QdiagEqual_RdiagSpecify) )
  write_rds(marss_fit_QdiagEqual_RdiagSpecify, path = 'marss_fit_QdiagEqual_RdiagSpecify.rds')
}
...
## Model kem_fit_QdiagUnequal_Restimate

```{r marss_QdiagUnequal_RdiagEqual}
...
if( file.exists( 'marss_fit_QdiagUnequal_RdiagEqual.rds' )){
  marss_fit_QdiagUnequal_RdiagEqual <- read_rds( 'marss_fit_QdiagUnequal_RdiagEqual.rds' )
} else {
  ...
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagUnequal_RdiagEqual  <- MARSS(Yt, model = model.gen, control = list(maxit =  1000, trace = 1), 
                                                            method = 'BFGS')
  
  write_rds(marss_fit_QdiagUnequal_RdiagEqual, path = 'marss_fit_QdiagUnequal_RdiagEqual.rds')
}
```


## Model kem_fit_QdiagUnequal_RdiagSpecify

```{r marss_QdiagUnequal_RdiagSpecify}

# Standardize the diagonal of gam18_rating for estimation
#  These standardized variance terms average 1 and form the diagonal of Q
# gam18_diag_std <- diag(gam18_rating$Vp)/mean(diag(gam18_rating$Vp))

if( file.exists( 'marss_fit_QdiagUnequal_RdiagSpecify.rds' )){
  marss_fit_QdiagUnequal_RdiagSpecify <- read_rds( 'marss_fit_QdiagUnequal_RdiagSpecify.rds' )
} else {
  # State variance matrix is based on the gam18_measPair$Vp
  Q <- matrix(list('0.1166336*q',0,0,0,0,
                   0,'1.3179593*q',0,0,0  ,
  ...
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagUnequal_RdiagSpecify  <- MARSS(Yt, model = model.gen, control = list(maxit =  1000, trace = 1), 
                                                            method = 'BFGS')
  
  write_rds(marss_fit_QdiagUnequal_RdiagSpecify, path = 'marss_fit_QdiagUnequal_RdiagSpecify.rds')
}
```

## Model kem_fit_QdiagEqual_RSeries

```{r set_marss_local_QdiagEqual_Rseries}


if ( file.exists( 'marss_fit_QdiagEqual_Rseries.rds' )){
  marss_fit_QdiagEqual_Rseries <- read_rds( 'marss_fit_QdiagEqual_Rseries.rds' )
} else {
  Q = 'diagonal and equal'
  ...
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagEqual_Rseries1  <- MARSS(Yt, model = model.gen, method = 'kem',
                                         control = list(maxit =  1000, trace = 1, conv.test.slope.tol = 0.05),
                                         inits = coef(marss_fit_QdiagEqual_Rseries))
  
  write_rds(marss_fit_QdiagEqual_Rseries, path = 'marss_fit_QdiagEqual_Rseries.rds')
}
...
## Model marss_fit_QdiagUnequal_Rseries

```{r set_marss_local_QdiagUnequal_Rseries}

# Standardize the diagonal of gam18_rating for estimation

gam18_diag_std <- diag(gam18_rating$Vp)/mean(diag(gam18_rating$Vp))

if( file.exists( 'marss_fit_QdiagUnequal_Rseries.rds' )){
  marss_fit_QdiagUnequal_Rseries <- read_rds( 'marss_fit_QdiagUnequal_Rseries.rds' )
} else {
  # State variance matrix is based on the gam18_measPair$Vp
  Q <- matrix(list('0.1166336*q',0,0,0,0,
                   0,'1.3179593*q',0,0,0  ,
  ...
  # Refit model: trace = 1 provides parameter etimates with each iteration with the corresponding log-likelihood
  marss_fit_QdiagUnequal_Rseries  <- MARSS(Yt, model = model.gen, control = list(maxit =  1000, trace = 1, 
                                                            conv.test.slope.tol = 0.05), method = 'kem',
                      inits = coef(marss_fit_QdiagUnequal_Rseries))
  
  write_rds(marss_fit_QdiagUnequal_Rseries, path = 'marss_fit_QdiagUnequal_Rseries.rds')
}
...
marss_compare <- data.frame( Q.Process     = c(rep('diagonal and equal', 2), rep('diagonal and unequal with multiplier',2),
                                             'diagonal and equal', 'diagonal and unequal with multiplier' ),
                             R.Measurement = c('estimated scalar', 'specified mean', 'estimated scalar', 'specified mean',
                                             rep('specified series', 2)),
                   'Method'         = c( 'BFGS', 'BFGS', 'BFGS', 'BFGS', 'kem', 'kem' ),
                   'Q_Process'      = format( c(marss_fit_QdiagEqual_RdiagEqual$par$Q,
                                                marss_fit_QdiagEqual_RdiagSpecify$par$Q,
                                                marss_fit_QdiagUnequal_RdiagEqual$par$Q,
                                                marss_fit_QdiagEqual_Rseries$par$Q,
                                                marss_fit_QdiagUnequal_RdiagSpecify$par$Q,
                                                marss_fit_QdiagUnequal_Rseries$par$Q), digits = 4),
                   'R_Measurement'  = c(format(marss_fit_QdiagEqual_RdiagEqual$par$R, digits = 4),
                                        format(marss_fit_QdiagEqual_RdiagSpecify$model$fixed$R, digits = 4),
                                        format(marss_fit_QdiagUnequal_RdiagEqual$par$R, digits = 4),
                                        format(mean(marss_fit_QdiagEqual_Rseries$marss$fixed$R[1,1,][ndx_ian]), digits = 4),
                                        rep('R[qualifier(t)]',2)),
                   'Likelihood'     = format(c(marss_fit_QdiagEqual_RdiagEqual$logLik, 
                                               marss_fit_QdiagEqual_RdiagSpecify$logLik,
                                               marss_fit_QdiagUnequal_RdiagEqual$logLik,
                                               marss_fit_QdiagEqual_Rseries$logLik,
                                               marss_fit_QdiagUnequal_RdiagSpecify$logLik,
                                               marss_fit_QdiagUnequal_Rseries$logLik), digits = 4),
                   'AIC'            = format(c(marss_fit_QdiagEqual_RdiagEqual$AIC,
                                               marss_fit_QdiagEqual_RdiagSpecify$AIC,
                                               marss_fit_QdiagUnequal_RdiagEqual$AIC,
                                               marss_fit_QdiagEqual_Rseries$AIC,
                                               marss_fit_QdiagUnequal_RdiagSpecify$AIC,
                                               marss_fit_QdiagUnequal_Rseries$AIC), digits = 4))
                                               
                   
marss_compare %>% 
  kable( caption = 'Table 5. Summary statistics from Multivariate Autoregressive State-Space estimation.',
         col.names = c('Process Variance, Q', 'Measurement Variance, R', 'Parameter Estimation Method', 
                       'Process Variance, Q', 
                       'Measurement Variance, R','Log-likelihood', 'Akaike Information Crieteria')) %>% 
  kable_styling( position = 'center', latex_options = 'scale down') %>% 
  column_spec(1, width = '12em' ) %>% 
  column_spec(3, width = '8em' ) %>%
  column_spec(6, width = '8em' ) %>% 
  add_header_above( c('Structure' = 2, ' ' ,'Characterization' = 2, 
                      ' ' = 1, ' ' = 1), align = 'center') %>% 
...
marss_fit <- marss_fit_QdiagEqual_Rseries
